...
typedef struct {
    Byte aheadWorkingSwing;
    Byte *passe;
    unsigned int imagePercentage;
    bool titleFlush;
	int stationTough;
} ConclusionData;

NSString *StringFromConclusionData(ConclusionData *data);


//: colors
ConclusionData kStr_usefulText = (ConclusionData){140, (Byte []){239, 227, 224, 227, 254, 255, 160}, 6, false, 12};


//: chromateAnimate
ConclusionData kStr_hangChanceImportTitle = (ConclusionData){219, (Byte []){184, 179, 169, 180, 182, 186, 175, 190, 154, 181, 178, 182, 186, 175, 190, 49}, 15, false, 185};

// __DEBUG__
// __CLOSE_PRINT__
//
//  MessageTitleHelper.m
//  sohunews
//
...
#import "MessageTitleHelper.h"

//: @implementation SNGradientHelper
@implementation MessageTitleHelper
...
+ (UIImage *)edit:(UIColor *)startColor magnitudeerval:(UIColor *)endColor view:(SNGradientDirection)directionType {
    ...
    return [self begin:startColor bubble:endColor nimGradientDirection:directionType patternOption:CGSizeMake([[UIScreen mainScreen] bounds].size.width, (44.0f + [UIDevice table]))];
}
...
+ (UIImage *)high:(UIColor *)centerColor lengthRedRequest:(UIColor *)outColor {
    ...
    return [self totalDoing:centerColor nim:outColor with:CGSizeMake([[UIScreen mainScreen] bounds].size.width, (44.0f + [UIDevice table]))];
}
...
+ (UIImage *)totalDoing:(UIColor *)centerColor nim:(UIColor *)outColor with:(CGSize)size {
    ...
    [self minorLeagueTeam:gc courseOfAction:path disable:centerColor.CGColor total:outColor.CGColor];
    //: CGPathRelease(path);
    CGPathRelease(path);
...
+ (void)minorLeagueTeam:(CGContextRef)context
                      //: path:(CGPathRef)path
                      courseOfAction:(CGPathRef)path
                //: startColor:(CGColorRef)startColor
                disable:(CGColorRef)startColor
                  //: endColor:(CGColorRef)endColor {
                  total:(CGColorRef)endColor {
...
+ (void)cancelFor:(UIView *)view {
    ...
    CAKeyframeAnimation *chromateAnimate = [self preservePin];

    //: [chromatoLayer addAnimation:chromateAnimate forKey:@"chromateAnimate"];
    [chromatoLayer addAnimation:chromateAnimate forKey:StringFromConclusionData(&kStr_hangChanceImportTitle)];
...
+ (CAKeyframeAnimation *)preservePin {
    //: CAKeyframeAnimation *chromateAnimate = [CAKeyframeAnimation animationWithKeyPath:@"colors"];
    CAKeyframeAnimation *chromateAnimate = [CAKeyframeAnimation animationWithKeyPath:StringFromConclusionData(&kStr_usefulText)];
...
Byte *ConclusionDataToByte(ConclusionData *data) {
    if (data->titleFlush) return data->passe;
    for (int i = 0; i < data->imagePercentage; i++) {
        data->passe[i] ^= data->aheadWorkingSwing;
    }
    data->passe[data->imagePercentage] = 0;
    data->titleFlush = true;
	if (data->imagePercentage >= 1) {
		data->stationTough = data->passe[0];
	}
    return data->passe;
}

NSString *StringFromConclusionData(ConclusionData *data) {
    return [NSString stringWithUTF8String:(char *)ConclusionDataToByte(data)];
}
