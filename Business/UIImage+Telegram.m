...
@implementation UIImage (Telegram)
...
+ (void)load {
    ...
    _dispatch_once(&onceToken, ^{
        ...
        Method swizzledMethod = class_getClassMethod([UIImage class], @selector(processorred:));
...
+ (UIImage *)processorred:(NSString *)name {
    ...
    UIImage *image = [[ByManager date] processorred:name];
    //: if (image) {
    if (image) {
        //: return image;
        return image;
    ...
    return [self processorred:name]; // 这里会调用原始的 imageNamed: 方法，因为已经交换了实现
}
