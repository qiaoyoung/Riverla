...
NSString *StringFromFogData(FogData *data);


//: bk_media_position_normal
FogData kStr_harshTitle = (FogData){117, (Byte []){23, 30, 42, 24, 16, 17, 28, 20, 42, 5, 26, 6, 28, 1, 28, 26, 27, 42, 27, 26, 7, 24, 20, 25, 38}, 24, false};


//: onTapMediaItemPicture:
FogData kStr_martText = (FogData){188, (Byte []){211, 210, 232, 221, 204, 241, 217, 216, 213, 221, 245, 200, 217, 209, 236, 213, 223, 200, 201, 206, 217, 134, 223}, 22, false};


//: message_send_camera
FogData kStr_passageData = (FogData){230, (Byte []){139, 131, 149, 149, 135, 129, 131, 185, 149, 131, 136, 130, 185, 133, 135, 139, 131, 148, 135, 224}, 19, false};


//: message_please_enter_content
FogData kStr_dessertValue = (FogData){20, (Byte []){121, 113, 103, 103, 117, 115, 113, 75, 100, 120, 113, 117, 103, 113, 75, 113, 122, 96, 113, 102, 75, 119, 123, 122, 96, 113, 122, 96, 169}, 28, false};


//: bk_media_picture_normal
FogData kStr_shotName = (FogData){37, (Byte []){71, 78, 122, 72, 64, 65, 76, 68, 122, 85, 76, 70, 81, 80, 87, 64, 122, 75, 74, 87, 72, 68, 73, 2}, 23, false};


//: bk_media_shoot_normal
FogData kStr_liveTitle = (FogData){160, (Byte []){194, 203, 255, 205, 197, 196, 201, 193, 255, 211, 200, 207, 207, 212, 255, 206, 207, 210, 205, 193, 204, 249}, 21, false};


//: {11,11,9,15}
FogData kStr_hindValue = (FogData){38, (Byte []){93, 23, 23, 10, 23, 23, 10, 31, 10, 23, 19, 91, 28}, 12, false};


//: {3,3,3,8}
FogData kStr_elderTitle = (FogData){200, (Byte []){179, 251, 228, 251, 228, 251, 228, 240, 181, 81}, 9, false};


//: {11,15,9,9}
FogData kStr_commanderContent = (FogData){177, (Byte []){202, 128, 128, 157, 128, 132, 157, 136, 157, 136, 204, 239}, 11, false};


//: #333333
FogData kStr_overlookGradName = (FogData){66, (Byte []){97, 113, 113, 113, 113, 113, 113, 132}, 7, false};


//: {10,10,10,10}
FogData kStr_mootMaxName = (FogData){6, (Byte []){125, 55, 54, 42, 55, 54, 42, 55, 54, 42, 55, 54, 123, 200}, 13, false};


//: onTapMediaItemShoot:
FogData kStr_chantName = (FogData){64, (Byte []){47, 46, 20, 33, 48, 13, 37, 36, 41, 33, 9, 52, 37, 45, 19, 40, 47, 47, 52, 122, 202}, 20, false};


//: message_send_album
FogData kStr_mythData = (FogData){180, (Byte []){217, 209, 199, 199, 213, 211, 209, 235, 199, 209, 218, 208, 235, 213, 216, 214, 193, 217, 9}, 18, false};


//: {8,12,8,12}
FogData kStr_fragileUpgradeName = (FogData){226, (Byte []){153, 218, 206, 211, 208, 206, 218, 206, 211, 208, 159, 213}, 11, false};


//: {9,11,9,15}
FogData kStr_continuousValue = (FogData){136, (Byte []){243, 177, 164, 185, 185, 164, 177, 164, 185, 189, 245, 124}, 11, false};


//: onTapMediaItemLocation:
FogData kStr_brigWindowValue = (FogData){242, (Byte []){157, 156, 166, 147, 130, 191, 151, 150, 155, 147, 187, 134, 151, 159, 190, 157, 145, 147, 134, 155, 157, 156, 200, 190}, 23, false};


//: onTapMenuItemCopy:
FogData kStr_lowerGradTitle = (FogData){240, (Byte []){159, 158, 164, 145, 128, 189, 149, 158, 133, 185, 132, 149, 157, 179, 159, 128, 137, 202, 179}, 18, false};


//: {9,15,9,9}
FogData kStr_vaccineValue = (FogData){145, (Byte []){234, 168, 189, 160, 164, 189, 168, 189, 168, 236, 92}, 10, false};


//: {3,8,3,3}
FogData kStr_guitarBitterValue = (FogData){9, (Byte []){114, 58, 37, 49, 37, 58, 37, 58, 116, 234}, 9, false};


//: bk_media_picture_pressed
FogData kStr_indexCivilValue = (FogData){64, (Byte []){34, 43, 31, 45, 37, 36, 41, 33, 31, 48, 41, 35, 52, 53, 50, 37, 31, 48, 50, 37, 51, 51, 37, 36, 104}, 24, false};


//: {8,20,8,20}
FogData kStr_sparkUntilName = (FogData){52, (Byte []){79, 12, 24, 6, 4, 24, 12, 24, 6, 4, 73, 143}, 11, false};


//: icon_session_time_bg
FogData kStr_hoseData = (FogData){122, (Byte []){19, 25, 21, 20, 37, 9, 31, 9, 9, 19, 21, 20, 37, 14, 19, 23, 31, 37, 24, 29, 237}, 20, false};


//: Audios
FogData kStr_improveName = (FogData){247, (Byte []){182, 130, 147, 158, 152, 132, 235}, 6, false};
...
- (instancetype)initRecordKey:(BOOL)isRight;
...
- (NSArray *)defaultOf
{
    ...
    return @[[ImageRecent bar:StringFromFogData(&kStr_martText)
           //: normalImage:[UIImage imageNamed:@"bk_media_picture_normal"]
           at:[UIImage imageNamed:StringFromFogData(&kStr_shotName)]
         //: selectedImage:[UIImage imageNamed:@"bk_media_picture_normal"]
         provider:[UIImage imageNamed:StringFromFogData(&kStr_shotName)]
                 //: title:[FFFLanguageManager getTextWithKey:@"message_send_album"]],//@"相册".
                 share:[ItemManager sessionInformation:StringFromFogData(&kStr_mythData)]],//@"相册".

    //: [FFFMediaItem item:@"onTapMediaItemShoot:"
    [ImageRecent bar:StringFromFogData(&kStr_chantName)
           //: normalImage:[UIImage imageNamed:@"bk_media_shoot_normal"]
           at:[UIImage imageNamed:StringFromFogData(&kStr_liveTitle)]
         //: selectedImage:[UIImage imageNamed:@"bk_media_shoot_normal"]
         provider:[UIImage imageNamed:StringFromFogData(&kStr_liveTitle)]
                 //: title:[FFFLanguageManager getTextWithKey:@"message_send_camera"]],//@"拍摄".
                 share:[ItemManager sessionInformation:StringFromFogData(&kStr_passageData)]],//@"拍摄".

    //: [FFFMediaItem item:@"onTapMediaItemLocation:"
    [ImageRecent bar:StringFromFogData(&kStr_brigWindowValue)
           //: normalImage:[UIImage imageNamed:@"bk_media_position_normal"]
           at:[UIImage imageNamed:StringFromFogData(&kStr_harshTitle)]
         //: selectedImage:[UIImage imageNamed:@"bk_media_position_normal"]
         provider:[UIImage imageNamed:StringFromFogData(&kStr_harshTitle)]
                 //: title:[FFFLanguageManager getTextWithKey:@"Audios"]],//@"位置".
                 share:[ItemManager sessionInformation:StringFromFogData(&kStr_improveName)]],//@"位置".
...
- (NSArray *)of:(NIMMessage *)message
{
    ...
    if (message.messageType == NIMMessageTypeText)
    {
        ...
        [menuItems addObject:[ImageRecent bar:StringFromFogData(&kStr_lowerGradTitle)
                                    //: normalImage:[UIImage imageNamed:@"bk_media_picture_normal"]
                                    at:[UIImage imageNamed:StringFromFogData(&kStr_shotName)]
                                  //: selectedImage:[UIImage imageNamed:@"bk_media_picture_pressed"]
                                  provider:[UIImage imageNamed:StringFromFogData(&kStr_indexCivilValue)]
                                          //: title:[FFFLanguageManager getTextWithKey:@"复制"]]];
                                          share:[ItemManager sessionInformation:@"复制"]]];
    }

//    ImageRecent *delItem = [ImageRecent item:@"onTapMenuItemDelete:"
//                                normalImage:[UIImage imageNamed:@"bk_media_shoot_normal"]
...
- (void)clean
{
    ...
    _placeholder = [ItemManager sessionInformation:StringFromFogData(&kStr_dessertValue)];//@"请输入消息".;
    //: _inputMaxLength = 1000;
    _inputMaxLength = 1000;
    //: _nickFont = [UIFont boldSystemFontOfSize:15];
    _nickFont = [UIFont boldSystemFontOfSize:15];
    //: _nickColor = [UIColor colorWithHexString:@"#333333"];
    _nickColor = [UIColor withMax:StringFromFogData(&kStr_overlookGradName)];
    ...
    _leftBubbleSettings = [[UserMargin alloc] initRecordKey:NO];
    //: _rightBubbleSettings = [[FFFKitSettings alloc] init:YES];
    _rightBubbleSettings = [[UserMargin alloc] initRecordKey:YES];
}
...
- (WorkerSetting *)with:(NIMMessage *)message
{
    ...
    UserMargin *settings = message.isOutgoingMsg? self.rightBubbleSettings : self.leftBubbleSettings;
...
- (void)ting
{
    ...
    _repliedSetting = [[WorkerSetting alloc] init];
    //: _repliedSetting.contentInsets = _isRight? UIEdgeInsetsFromString(@"{8,12,8,12}") : UIEdgeInsetsFromString(@"{8,12,8,12}");
    _repliedSetting.contentInsets = _isRight? UIEdgeInsetsFromString(StringFromFogData(&kStr_fragileUpgradeName)) : UIEdgeInsetsFromString(StringFromFogData(&kStr_fragileUpgradeName));
...
- (void)child
{
    ...
    _textSetting = [[WorkerSetting alloc] initSession:_isRight];
    //: _textSetting.contentInsets = _isRight? UIEdgeInsetsFromString(@"{8,12,8,12}") : UIEdgeInsetsFromString(@"{8,12,8,12}");
    _textSetting.contentInsets = _isRight? UIEdgeInsetsFromString(StringFromFogData(&kStr_fragileUpgradeName)) : UIEdgeInsetsFromString(StringFromFogData(&kStr_fragileUpgradeName));
...
- (void)can
{
    ...
    _audioSetting = [[WorkerSetting alloc] initSession:_isRight];
    //: _audioSetting.contentInsets = _isRight? UIEdgeInsetsFromString(@"{8,12,8,12}") : UIEdgeInsetsFromString(@"{8,12,8,12}");
    _audioSetting.contentInsets = _isRight? UIEdgeInsetsFromString(StringFromFogData(&kStr_fragileUpgradeName)) : UIEdgeInsetsFromString(StringFromFogData(&kStr_fragileUpgradeName));
...
- (void)should
{
    ...
    _videoSetting = [[WorkerSetting alloc] initSession:_isRight];
    //: _videoSetting.contentInsets = _isRight? UIEdgeInsetsFromString(@"{3,3,3,8}") : UIEdgeInsetsFromString(@"{3,8,3,3}");
    _videoSetting.contentInsets = _isRight? UIEdgeInsetsFromString(StringFromFogData(&kStr_elderTitle)) : UIEdgeInsetsFromString(StringFromFogData(&kStr_guitarBitterValue));
...
- (void)elect
{
    ...
    _fileSetting = [[WorkerSetting alloc] initSession:_isRight];
    //: _fileSetting.contentInsets = _isRight? UIEdgeInsetsFromString(@"{3,3,3,8}") : UIEdgeInsetsFromString(@"{3,8,3,3}");
    _fileSetting.contentInsets = _isRight? UIEdgeInsetsFromString(StringFromFogData(&kStr_elderTitle)) : UIEdgeInsetsFromString(StringFromFogData(&kStr_guitarBitterValue));
...
- (void)buttonShow
{
    ...
    _imageSetting = [[WorkerSetting alloc] initSession:_isRight];
    //: _imageSetting.contentInsets = _isRight? UIEdgeInsetsFromString(@"{3,3,3,8}") : UIEdgeInsetsFromString(@"{3,8,3,3}");
    _imageSetting.contentInsets = _isRight? UIEdgeInsetsFromString(StringFromFogData(&kStr_elderTitle)) : UIEdgeInsetsFromString(StringFromFogData(&kStr_guitarBitterValue));
...
- (void)sign
{
    ...
    _locationSetting = [[WorkerSetting alloc] initSession:_isRight];
    //: _locationSetting.contentInsets = _isRight? UIEdgeInsetsFromString(@"{3,3,3,8}") : UIEdgeInsetsFromString(@"{3,8,3,3}");
    _locationSetting.contentInsets = _isRight? UIEdgeInsetsFromString(StringFromFogData(&kStr_elderTitle)) : UIEdgeInsetsFromString(StringFromFogData(&kStr_guitarBitterValue));
...
- (void)sessionColor
{
    ...
    _tipSetting = [[WorkerSetting alloc] initSession:_isRight];
    ...
    UIImage *backgroundImage = [[UIImage imageNamed:StringFromFogData(&kStr_hoseData)] resizableImageWithCapInsets:UIEdgeInsetsFromString(StringFromFogData(&kStr_sparkUntilName)) resizingMode:UIImageResizingModeStretch];;
...
- (void)record
{
    ...
    _rtcCallRecordSetting = [[WorkerSetting alloc] initSession:_isRight];
    //: _rtcCallRecordSetting.contentInsets = _isRight? UIEdgeInsetsFromString(@"{9,11,9,15}") : UIEdgeInsetsFromString(@"{9,15,9,9}");
    _rtcCallRecordSetting.contentInsets = _isRight? UIEdgeInsetsFromString(StringFromFogData(&kStr_continuousValue)) : UIEdgeInsetsFromString(StringFromFogData(&kStr_vaccineValue));
...
- (void)object
{
    ...
    _unsupportSetting = [[WorkerSetting alloc] initSession:_isRight];
    //: _unsupportSetting.contentInsets = _isRight? UIEdgeInsetsFromString(@"{10,10,10,10}") : UIEdgeInsetsFromString(@"{10,10,10,10}");
    _unsupportSetting.contentInsets = _isRight? UIEdgeInsetsFromString(StringFromFogData(&kStr_mootMaxName)) : UIEdgeInsetsFromString(StringFromFogData(&kStr_mootMaxName));
...
- (void)foreword
{
    ...
    _teamNotificationSetting = [[WorkerSetting alloc] initSession:_isRight];
...
- (void)with
{
    ...
    _superTeamNotificationSetting = [[WorkerSetting alloc] initSession:_isRight];
...
- (void)team
{
    ...
    _chatroomNotificationSetting = [[WorkerSetting alloc] initSession:_isRight];
...
- (void)than
{
    ...
    _netcallNotificationSetting = [[WorkerSetting alloc] initSession:_isRight];
    //: _netcallNotificationSetting.contentInsets = _isRight? UIEdgeInsetsFromString(@"{11,11,9,15}") : UIEdgeInsetsFromString(@"{11,15,9,9}");
    _netcallNotificationSetting.contentInsets = _isRight? UIEdgeInsetsFromString(StringFromFogData(&kStr_hindValue)) : UIEdgeInsetsFromString(StringFromFogData(&kStr_commanderContent));
...
Byte *FogDataToByte(FogData *data) {
    if (data->comedyBlue) return data->ax;
    for (int i = 0; i < data->purpose; i++) {
        data->ax[i] ^= data->howModel;
    }
    data->ax[data->purpose] = 0;
    data->comedyBlue = true;
    return data->ax;
}

NSString *StringFromFogData(FogData *data) {
    return [NSString stringWithUTF8String:(char *)FogDataToByte(data)];
}
