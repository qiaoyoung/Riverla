...
@interface ActionView () <TeamCenter>
...
@property (nonatomic, strong) NSMutableArray<ByWithView*> *items;
...
@property (nonatomic, strong) VisualisationView *touchReceiver;
...
static NSInteger const let_stateValue = 2;
//: static NSTimeInterval const DEFAULT_TIME_INTERVAL = 4.0;
static NSTimeInterval const let_eventValue = 4.0;
//: static NSTimeInterval const DEFAULT_TIME_DURATION = 1.0;
static NSTimeInterval const let_kitTitle = 1.0;
//: static float const DEFAULT_SCROLL_SPEED = 40.0f;
static float const let_modelTitleKitName = 40.0f;
//: static float const DEFAULT_ITEM_SPACING = 20.0f;
static float const let_objectName = 20.0f;
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    if (self = [super initWithCoder:aDecoder]) {
        ...
        _timeIntervalPerScroll = let_eventValue;
        //: _timeDurationPerScroll = DEFAULT_TIME_DURATION;
        _timeDurationPerScroll = let_kitTitle;
        //: _scrollSpeed = DEFAULT_SCROLL_SPEED;
        _scrollSpeed = let_modelTitleKitName;
        //: _itemSpacing = DEFAULT_ITEM_SPACING;
        _itemSpacing = let_objectName;
        //: _touchEnabled = NO;
        _touchEnabled = NO;
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 ...
                                                 selector:@selector(pastCellNotification:)
...
- (void)pastCellNotification:(NSNotification*)notification {
...
- (void)titles:(NSNotification*)notification {
...
- (void)backgroundAll {
    ...
    //: } else {
        ...
        if ([_delegate respondsToSelector:@selector(ranges:)]) {
            //: self.visibleItemCount = [_delegate numberOfVisibleItemsForMarqueeView:self];
            self.visibleItemCount = [_delegate ranges:self];
        ...
        //: } else {
            ...
            self.visibleItemCount = let_stateValue;
        }
    }
    ...
    for (int i = 0; i < _visibleItemCount + 2; i++) {
        ...
        ByWithView *itemView = [[ByWithView alloc] init];
    ...
    if (_direction == UUMarqueeViewDirectionLeftward) {
        ...
        for (int i = 0; i < _items.count; i++) {
            ...
            //: } else {
                ...
                _items[index].width = [self reply:_items[index].tag];
                ...
                [self bubbleMessage:_items[index] fromIndex:_items[index].tag];
            }
        }
    //: } else {
    } else {
        ...
        if (_useDynamicHeight) {
            ...
            for (int i = 0; i < _items.count; i++) {
                ...
                if (i == _items.count - 1) {
                    ...
                    _items[index].height = [self toteUp:_items[index].tag];
        ...
        //: } else {
            ...
            if ([_delegate respondsToSelector:@selector(unwelcomeWith:)]) {
                //: dataCount = [_delegate numberOfDataForMarqueeView:self];
                dataCount = [_delegate unwelcomeWith:self];
            }
...
- (void)bubbleMessage:(ByWithView*)itemView fromIndex:(NSInteger)index {
    //: if (index < 0) {
    if (index < 0) {
        //: [itemView clear];
        [itemView playCollection];
    }
    ...
    if (index >= 0) {
        ...
        if ([_delegate respondsToSelector:@selector(notModify:cipher:imageShare:)]) {
            //: [_delegate updateItemView:itemView atIndex:index forMarqueeView:self];
            [_delegate notModify:itemView cipher:index imageShare:self];
        }
    }
}
...
- (void)keyed:(NSTimer *)timer {
    ...
    dispatch_async(dispatch_get_main_queue(), ^() {
        ...
        if (_direction == UUMarqueeViewDirectionLeftward) {
            ...
            _items[_firstItemIndex].width = [self reply:_items[_firstItemIndex].tag];
            ...
            if (firstItemWidth != nextItemWidth) {
                ...
                [_items[_firstItemIndex] playCollection];
            }
            //: [self updateItemView:_items[_firstItemIndex] atIndex:_items[_firstItemIndex].tag];
            [self bubbleMessage:_items[_firstItemIndex] fromIndex:_items[_firstItemIndex].tag];
        ...
        //: } else {
            ...
            if (_useDynamicHeight) {
                ...
                _items[_firstItemIndex].height = [self toteUp:_items[_firstItemIndex].tag];
...
- (void)view {
    ...
    if (_touchEnabled) {
        ...
        if (!_touchReceiver) {
            ...
            self.touchReceiver = [[VisualisationView alloc] init];
...
- (void)top:(CGPoint)point {
    ...
    for (ByWithView *itemView in _items) {
        ...
        if ([itemView.layer.presentationLayer hitTest:point]) {
            ...
            if (dataCount > 0 && itemView.tag >= 0 && itemView.tag < dataCount) {
                ...
                if ([self.delegate respondsToSelector:@selector(itemName:emotion:)]) {
                    //: [self.delegate didTouchItemViewAtIndex:itemView.tag forMarqueeView:self];
                    [self.delegate itemName:itemView.tag emotion:self];
                }
            }
            //: break;
            break;
...
@implementation VisualisationView
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if (_touchDelegate) {
        ...
        [_touchDelegate top:touchLocation];
    }
}
