...
@interface FaceMessageApp ()

//: @end
@end

//: @implementation NIMCardMemberItem
@implementation FaceMessageApp
...
- (UIImage *)imageNormal{
    ...
    OriginBy *info = [[Notice fullKit] value:self.userId manager:nil];
...
- (NSString *)title {
    ...
    return [UtilizerUtil along:self.userId toStop:session];
}

//: - (NSString *)imageUrl{
- (NSString *)imageUrl{
    //: return [[MyUserKit sharedKit] infoByUser:self.userId option:nil].avatarUrlString;
    return [[Notice fullKit] value:self.userId manager:nil].avatarUrlString;
}
...
- (BOOL)isEqual:(id)object{
    ...
    if (![object isKindOfClass:[PunchCardShow class]]) {
        //: return NO;
        return NO;
    }
    //: FFFTeamCardMemberItem *obj = (FFFTeamCardMemberItem*)object;
    PunchCardShow *obj = (PunchCardShow*)object;
