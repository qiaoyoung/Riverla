...
//: content
Byte kStr_capabilityContent[] = {36, 7, 9, 26, 254, 177, 107, 250, 133, 116, 110, 101, 116, 110, 111, 99, 127};


//: default
Byte kStr_steadControlTitle[] = {51, 7, 13, 171, 183, 84, 69, 41, 251, 3, 219, 136, 155, 116, 108, 117, 97, 102, 101, 100, 65};


//: Chartlet
Byte kStr_barNimFlueText[] = {21, 8, 5, 244, 157, 116, 101, 108, 116, 114, 97, 104, 67, 230};
...
+ (UIImage *)chartlet:(NSString *)imageName read:(NSString *)chartletId{
    ...
    if ([chartletId isEqualToString:StringFromDisabledData(kStr_steadControlTitle)]) {
        //: return [UIImage imageNamed:imageName];
        return [UIImage imageNamed:imageName];
    }
    //: NSString *subDirectory = [NSString stringWithFormat:@"%@/%@/%@",@"Chartlet",chartletId,@"content"];
    NSString *subDirectory = [NSString stringWithFormat:@"%@/%@/%@",StringFromDisabledData(kStr_barNimFlueText),chartletId,StringFromDisabledData(kStr_capabilityContent)];
    //先拿2倍图
...
- (UIImage *)forLabel
{
    ...
    CGFloat pixels = [[TableDevice record] ting];
    //: UIImage * image = [self nim_imageForUpload:pixels];
    UIImage * image = [self outMobileTrack:pixels];
...
- (UIImage *)outMobileTrack: (CGFloat)suggestPixels
{
...
- (UIImage *)push: (CGFloat)maxPixels
{
    ...
    return [self session:CGSizeMake(newSizeWidth, newSizeHeight)];
}

//内缩放，一条变等于最长边，另外一条小于等于最长边
//: - (UIImage *)nim_scaleToSize:(CGSize)newSize
- (UIImage *)session:(CGSize)newSize
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
    ...
    return [self arrayOptionSize:size];
}

//: - (UIImage *)nim_drawImageWithSize: (CGSize)size
- (UIImage *)arrayOptionSize: (CGSize)size
{
...
- (UIImage *)enable:(CGSize)targetSize
{
    // 裁剪两边
...
Byte * DisabledDataToCache(Byte *data) {
    int balanceJack = data[0];
    int screenRecent = data[1];
    int primWith = data[2];
    if (!balanceJack) return data + primWith;
    for (int i = 0; i < screenRecent / 2; i++) {
        int begin = primWith + i;
        int end = primWith + screenRecent - i - 1;
        Byte temp = data[begin];
        data[begin] = data[end];
        data[end] = temp;
    }
    data[0] = 0;
    data[primWith + screenRecent] = 0;
    return data + primWith;
}
