...
- (CloakView *)maskView {
    ...
    if (_maskView == nil) {
        ...
        _maskView = [[CloakView alloc] initWithFrame:self.bounds];
...
- (UIImageView *)tap {
    ...
    if (_imageView == nil) {
        ...
        [[self color] addSubview:_imageView];
    }
    //: return _imageView;
    return _imageView;
}
...
- (void)setCropRatio:(KIImageCropRatio)cropRatio {
    ...
    if (cropRatio != _cropRatio) {
        ...
        [self commentInRatio:_cropRatio];
    }
}
...
- (void)commentInRatio:(KIImageCropRatio)ratio {
...
- (void)setImage:(UIImage *)image {
    ...
    if (_image != image) {
        ...
        [[self tap] setImage:_image];

        //: [[self imageView] setFrame:CGRectMake(0, 0, _image.size.width, _image.size.height)];
        [[self tap] setFrame:CGRectMake(0, 0, _image.size.width, _image.size.height)];
...
- (void)read {
    ...
    [[self color] setMinimumZoomScale:min];
    //: [[self scrollView] setMaximumZoomScale:max+5.0f];
    [[self color] setMaximumZoomScale:max+5.0f];

    //: [[self scrollView] setZoomScale:min animated:YES];
    [[self color] setZoomScale:min animated:YES];

    //: [[self scrollView] setContentInset:_imageViewInsets];
    [[self color] setContentInset:_imageViewInsets];
    //: [[self scrollView] setContentOffset:CGPointMake(-_imageViewInsets.left, -_imageViewInsets.top) animated:YES];
    [[self color] setContentOffset:CGPointMake(-_imageViewInsets.left, -_imageViewInsets.top) animated:YES];
}
...
- (void)layoutSubviews {
    ...
    [(CloakView*)[self maskView] setIndex:[self viewInfo]];
    //: [[self scrollView] setFrame:self.bounds];
    [[self color] setFrame:self.bounds];
...
- (UIImage *)scaleColor {
    ...
    CGFloat zoomScale = [self color].zoomScale;

    //: CGFloat offsetX = [self scrollView].contentOffset.x;
    CGFloat offsetX = [self color].contentOffset.x;
    //: CGFloat offsetY = [self scrollView].contentOffset.y;
    CGFloat offsetY = [self color].contentOffset.y;
    ...
    image = [self of:image mentalPicture:imgSize closeTeamOpen:YES];
...
- (UIImage*)of:(UIImage*)image mentalPicture:(CGSize)newSize closeTeamOpen:(BOOL)opaque
{
