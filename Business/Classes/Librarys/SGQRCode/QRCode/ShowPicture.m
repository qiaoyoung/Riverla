...
@interface ShowPicture () <AVCaptureMetadataOutputObjectsDelegate, AVCaptureVideoDataOutputSampleBufferDelegate>
{
    //: SGSoundEffect *soundEffect;
    LayerEffect *soundEffect;
}
...
- (void)setDelegate:(id<RecentDelegate>)delegate {
    //: _delegate = delegate;
    _delegate = delegate;
...
- (void)setSampleBufferDelegate:(id<MarginOfErrorDelegate>)sampleBufferDelegate {
...
- (void)replyEffect:(NSString *)name {
    ...
    NSString *voicePath = [[[ByManager date] item] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@", name]];
    ...
    soundEffect = [LayerEffect even:voicePath];
    //: [soundEffect play];
    [soundEffect labelPlay];
}
...
- (AVCaptureSession *)session {
    ...
    if (!_session) {
        ...
        _session.sessionPreset = [self messageShowPreset];
    }
    //: return _session;
    return _session;
}
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    ...
    if (metadataObjects != nil && metadataObjects.count > 0) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(showPin:message:)]) {
                //: [self.delegate scanCode:self result:resultString];
                [self.delegate showPin:self message:resultString];
            }
        //: });
        });
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self.sampleBufferDelegate && [self.sampleBufferDelegate respondsToSelector:@selector(writtenCommunication:key:)]) {
            //: [self.sampleBufferDelegate scanCode:self brightness:brightnessValue];
            [self.sampleBufferDelegate writtenCommunication:self key:brightnessValue];
        }
    //: });
    });
}
