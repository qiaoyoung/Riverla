...
#import "EndorseView.h"

//: @interface TPPasswordTextView ()
@interface EndorseView ()
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [textField addTarget:self action:@selector(sessionChange:) forControlEvents:UIControlEventEditingChanged];
...
- (void)background {
    ...
    [self sessionChange:self.textField];
}
...
- (void)sessionChange:(UITextField *)textField {
    ...
    if (password.length == self.dataSource.count)
    {
        ...
        if (self.autoHideKeyboard) {
            //: [self hideKeyboard];
            [self twentyFourHourPeriod];//隐藏键盘
        }
    }
