...
@implementation TrademarkUtil

//: + (NSString *)fileMD5:(NSString *)filepath
+ (NSString *)sight:(NSString *)filepath
{
    //: NSString *md5 = nil;
    NSString *md5 = nil;
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:filepath])
    {
        ...
        unsigned long long fileSize = [self range:filepath];
        //: if(fileSize > 200 * 1024 * 1024) {
        if(fileSize > 200 * 1024 * 1024) {
            //: md5 = [self bigFileMD5:filepath];
            md5 = [self dosage:filepath];
        //: }else {
        }else {
...
+ (void)accumulation:(NSString *)filepath notice:(void(^)(NSString *MD5))completion;
{
    //: dispatch_block_t block = ^(){
    dispatch_block_t block = ^(){
        //: NSString *md5 = [self fileMD5:filepath];
        NSString *md5 = [self sight:filepath];
        //: if(completion) {
        if(completion) {
            //: completion(md5);
            completion(md5);
...
+ (NSString *)dosage:(NSString *)filePath
{
...
+ (unsigned long long)range:(NSString *)filepath
{
