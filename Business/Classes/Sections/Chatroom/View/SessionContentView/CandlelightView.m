...
@implementation CandlelightView
...
- (void)erase:(NIMMessageModel *)data
{
    //: [super refresh:data];
    [super refresh:data];
...
- (UIImage *)back:(UIControlState)state direct:(BOOL)outgoing
{
    //: return nil;
    return nil;
}
...
- (void)dismiss:(NIMMessageModel *)data
{
...
- (void)inChild:(NIMKitRobotTemplateItem *)item
           //: inView:(UIView *)view
           view:(UIView *)view
{
    ...
    switch (item.itemType) {
        ...
        case NIMKitRobotTemplateItemTypeText:
        {
            ...
            ShowTitleView *label = [self genLabel];
        ...
        case NIMKitRobotTemplateItemTypeLinkBlock:
        {
            ...
            FoamButton *button = [self session];
...
- (CGSize)fits:(CGSize)size
{
    //: CGFloat height = [NTESSessionRobotContentView itemSpacing];
    CGFloat height = [CandlelightView itemSpacing];
    //: CGFloat width = 0;;
    CGFloat width = 0;;
    ...
    for (UIView *view in self.subviews) {
        ...
        if (view == self.continueButton)
        {
            ...
            height += [CandlelightView continueItemSpacing];
        }
        //: else
        else
        {
            //: height += [NTESSessionRobotContentView itemSpacing];
            height += [CandlelightView itemSpacing];
        }
    }
...
- (void)upses:(FoamButton *)button
{
    ...
    else
    {
        ...
        for (ShowTitleView *label in button.subviews)
        {
            //: if ([label isKindOfClass:[M80AttributedLabel class]])
            if ([label isKindOfClass:[ShowTitleView class]])
            {
...
- (void)keyImage
{
    ...
    [self should:self empty:self.model.contentViewInsets];

    //: UIEdgeInsets contentInsets = self.model.contentViewInsets;
    UIEdgeInsets contentInsets = self.model.contentViewInsets;
    //: CGFloat top = [NTESSessionRobotContentView itemSpacing] + contentInsets.top;
    CGFloat top = [CandlelightView itemSpacing] + contentInsets.top;
    //: for (UIView *view in self.subviews)
    for (UIView *view in self.subviews)
    {
        //: if (view == self.bubbleImageView)
        ...
        top = view.bottom + [CandlelightView itemSpacing];
    }
}


//: - (void)resizeAllSubView:(UIView *)superView insets:(UIEdgeInsets)insets
- (void)should:(UIView *)superView empty:(UIEdgeInsets)insets
{
    ...
    for (UIView *subView in superView.subviews)
    {
        ...
        if (subView.height == 0)
        {
            ...
            if ([subView isKindOfClass:[ShowTitleView class]])
            {
                //: M80AttributedLabel *label = (M80AttributedLabel *)subView;
                ShowTitleView *label = (ShowTitleView *)subView;
            ...
            else if ([subView isKindOfClass:[FoamButton class]])
            {
                //: NTESSessionRobotButton *button = (NTESSessionRobotButton *)subView;
                FoamButton *button = (FoamButton *)subView;
                //: button.width = width;
                button.width = width;
...
- (void)views:(UIView *)view
{
    ...
    for (UIView *subView in view.subviews)
    {
        ...
        if ([subView isKindOfClass:[FoamButton class]])
        {
            //: NTESSessionRobotButton *btn = (NTESSessionRobotButton *)subView;
            FoamButton *btn = (FoamButton *)subView;
            //: btn.target = nil;
            btn.target = nil;
            //: btn.url = nil;
            btn.url = nil;
...
- (ShowTitleView *)at
{
    ...
    else
    {
        ...
        label = [[ShowTitleView alloc] initWithFrame:CGRectZero];
...
- (FoamButton *)session
{
    ...
    else
    {
        ...
        button = [[FoamButton alloc] initWithFrame:CGRectMake(0, 0, 0, 0)];
