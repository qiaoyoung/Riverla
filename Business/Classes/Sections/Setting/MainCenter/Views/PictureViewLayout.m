...
static CGFloat const let_nameValue = 0.1;
//: static CGFloat const MIN_PRESS_TO_BEGIN_EDITING_DURATION = 0.6;
static CGFloat const let_layerMessageText = 0.6;
...
@interface PictureViewLayout () <UIGestureRecognizerDelegate>

//: @property (nonatomic,readonly) id<LxGridViewDataSource> dataSource;
@property (nonatomic,readonly) id<AnglicismSource> dataSource;
//: @property (nonatomic,readonly) id<LxGridViewDelegateFlowLayout> delegate;
@property (nonatomic,readonly) id<ShouldLayout> delegate;
...
- (void)bluePicture
{
    ...
    _longPressGestureRecognizer = [[UILongPressGestureRecognizer alloc]initWithTarget:self action:@selector(inputted:)];
    //: _longPressGestureRecognizer.cancelsTouchesInView = NO;
    _longPressGestureRecognizer.cancelsTouchesInView = NO;
    //: _longPressGestureRecognizer.minimumPressDuration = PRESS_TO_MOVE_MIN_DURATION;
    _longPressGestureRecognizer.minimumPressDuration = let_nameValue;
    ...
    _panGestureRecognizer = [[UIPanGestureRecognizer alloc]initWithTarget:self action:@selector(detects:)];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(demonstrateActive:) name:UIApplicationWillResignActiveNotification object:nil];
}
...
- (void)inputted:(UILongPressGestureRecognizer *)longPress
{
    ...
    switch (longPress.state) {
        ...
        case UIGestureRecognizerStateBegan:
        {
            ...
            if (_displayLink == nil) {
                ...
                _displayLink = [CADisplayLink displayLinkWithTarget:self selector:@selector(found:)];
                ...
                _remainSecondsToBeginEditing = let_layerMessageText;
            }
            ...
            if ([self.delegate respondsToSelector:@selector(noeticRetentivity:label:message:)]) {
                //: [self.delegate collectionView:self.collectionView layout:self willBeginDraggingItemAtIndexPath:_movingItemIndexPath];
                [self.delegate noeticRetentivity:self.collectionView label:self message:_movingItemIndexPath];
            }
            ...
            TeamViewCell *sourceCell = (TeamViewCell *)sourceCollectionViewCell;
            ...
            UIView * highlightedSnapshotView = [sourceCell lightView];
            ...
            UIView * snapshotView = [sourceCell lightView];
            ...
            [UIView animateWithDuration:0
                             ...
                             completion:^(BOOL finished) {
                                 ...
                                 if (strongSelf) {
                                     ...
                                     if ([strongSelf.delegate respondsToSelector:@selector(recording:cancel:should:)]) {
                                         ...
                                         [strongSelf.delegate recording:strongSelf.collectionView cancel:strongSelf should:self->_movingItemIndexPath];
                                     }
                                 }
                             //: }];
                             }];
        ...
        case UIGestureRecognizerStateCancelled:
        {
            ...
            if (movingItemIndexPath) {
                ...
                if ([self.delegate respondsToSelector:@selector(member:confirmOf:pressed:)]) {
                    //: [self.delegate collectionView:self.collectionView layout:self willEndDraggingItemAtIndexPath:movingItemIndexPath];
                    [self.delegate member:self.collectionView confirmOf:self pressed:movingItemIndexPath];
                }
                ...
                [UIView animateWithDuration:0.2
                                 ...
                                 completion:^(BOOL finished) {
                                     ...
                                     if (strongSelf) {
                                         ...
                                         if ([strongSelf.delegate respondsToSelector:@selector(target:time:shortness:)]) {
                                             ...
                                             [strongSelf.delegate target:strongSelf.collectionView time:strongSelf shortness:movingItemIndexPath];
                                         }
                                     }
                                 //: }];
                                 }];
...
- (void)detects:(UIPanGestureRecognizer *)pan
{
    //: switch (pan.state) {
    switch (pan.state) {
        //: case UIGestureRecognizerStatePossible:
        ...
        case UIGestureRecognizerStateChanged:
        {
            ...
            if ([self.dataSource respondsToSelector:@selector(file:counterval:imageWayOfLife:)] && [self.dataSource file:self.collectionView counterval:sourceIndexPath imageWayOfLife:destinationIndexPath] == NO) {
                //: return;
                return;
            }

            //: if ([self.dataSource respondsToSelector:@selector(collectionView:itemAtIndexPath:willMoveToIndexPath:)]) {
            if ([self.dataSource respondsToSelector:@selector(link:positionPath:child:)]) {
                ...
                [self.dataSource link:self.collectionView positionPath:sourceIndexPath child:destinationIndexPath];
            }
            ...
            //: } completion:^(BOOL finished) {
                ...
                if ([strongSelf.dataSource respondsToSelector:@selector(completePath:namePath:collection:)]) {
                    ...
                    [strongSelf.dataSource completePath:strongSelf.collectionView namePath:sourceIndexPath collection:destinationIndexPath];
                }
            //: }];
            }];
        }
...
- (void)found:(CADisplayLink *)displayLink
{
...
- (void)demonstrateActive:(NSNotification *)notificaiton
{
