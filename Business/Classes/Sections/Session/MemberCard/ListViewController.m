...
NSString *StringFromInstitutionalizedData(Byte *data);        


//: canAddFriend
Byte kStr_chainData[] = {86, 12, 50, 5, 173, 49, 47, 60, 15, 50, 50, 20, 64, 55, 51, 60, 50, 81};


//: kNIMTeamListDataTeamMembersChanged
Byte kStr_backgroundTitle[] = {35, 34, 18, 9, 183, 249, 52, 39, 216, 89, 60, 55, 59, 66, 83, 79, 91, 58, 87, 97, 98, 50, 79, 98, 79, 66, 83, 79, 91, 59, 83, 91, 80, 83, 96, 97, 49, 86, 79, 92, 85, 83, 82, 203};


//: back_arrow_bl
Byte kStr_occasionallyContent[] = {21, 13, 85, 11, 11, 20, 187, 68, 67, 141, 221, 13, 12, 14, 22, 10, 12, 29, 29, 26, 34, 10, 13, 23, 70};


//: /team/getTeamSetting
Byte kStr_disableKillerName[] = {9, 20, 11, 7, 152, 228, 151, 36, 105, 90, 86, 98, 36, 92, 90, 105, 73, 90, 86, 98, 72, 90, 105, 105, 94, 99, 92, 249};


//: group_info_activity_team_member
Byte kStr_hotelTitle[] = {83, 31, 77, 13, 199, 103, 245, 232, 187, 8, 107, 102, 77, 26, 37, 34, 40, 35, 18, 28, 33, 25, 34, 18, 20, 22, 39, 28, 41, 28, 39, 44, 18, 39, 24, 20, 32, 18, 32, 24, 32, 21, 24, 37, 168};


//: data
Byte kStr_itTenderName[] = {36, 4, 89, 10, 27, 249, 149, 7, 247, 141, 11, 8, 27, 8, 40};


//: code
Byte kStr_wantName[] = {91, 4, 18, 8, 90, 249, 128, 149, 81, 93, 82, 83, 67};


//: 邀请你加入超大群
Byte kStr_mobileText[] = {43, 24, 53, 6, 154, 76, 180, 77, 75, 179, 122, 130, 175, 136, 107, 176, 85, 107, 176, 80, 112, 179, 129, 80, 176, 111, 114, 178, 137, 111, 45};


//: 邀请你加入讨论组
Byte kStr_illnessName[] = {77, 24, 71, 7, 212, 26, 150, 162, 59, 57, 161, 104, 112, 157, 118, 89, 158, 67, 89, 158, 62, 94, 161, 103, 97, 161, 103, 115, 160, 116, 61, 226};


//: group_info_activity_op_failed
Byte kStr_thoroughName[] = {21, 29, 55, 13, 95, 154, 88, 104, 96, 141, 196, 209, 151, 48, 59, 56, 62, 57, 40, 50, 55, 47, 56, 40, 42, 44, 61, 50, 63, 50, 61, 66, 40, 56, 57, 40, 47, 42, 50, 53, 46, 45, 115};


//: attach
Byte kStr_nabTitle[] = {86, 6, 63, 13, 175, 245, 254, 140, 82, 232, 82, 14, 121, 34, 53, 53, 34, 36, 41, 176};


//: 扩展消息
Byte kStr_twentiethTitle[] = {99, 12, 34, 10, 220, 65, 79, 121, 132, 63, 196, 103, 135, 195, 143, 115, 196, 148, 102, 196, 95, 141, 238};


//: group_member_info_activity_team_creator
Byte kStr_yardHotelData[] = {4, 39, 62, 9, 134, 43, 6, 104, 118, 41, 52, 49, 55, 50, 33, 47, 39, 47, 36, 39, 52, 33, 43, 48, 40, 49, 33, 35, 37, 54, 43, 56, 43, 54, 59, 33, 54, 39, 35, 47, 33, 37, 52, 39, 35, 54, 49, 52, 255};


//: ic_group_addmember
Byte kStr_budgeTitle[] = {45, 18, 4, 10, 161, 57, 173, 142, 59, 35, 101, 95, 91, 99, 110, 107, 113, 108, 91, 93, 96, 96, 105, 97, 105, 94, 97, 110, 122};


//: modify_activity_modify_success
Byte kStr_egoWithName[] = {25, 30, 6, 13, 191, 12, 165, 181, 68, 171, 78, 33, 113, 103, 105, 94, 99, 96, 115, 89, 91, 93, 110, 99, 112, 99, 110, 115, 89, 103, 105, 94, 99, 96, 115, 89, 109, 111, 93, 93, 95, 109, 109, 130};


//: postscript
Byte kStr_dryTitle[] = {90, 10, 30, 14, 101, 75, 95, 187, 88, 166, 91, 214, 92, 112, 82, 81, 85, 86, 85, 69, 84, 75, 82, 86, 140};


//: 邀请你加入高级群
Byte kStr_effectivePatientValue[] = {38, 24, 53, 10, 235, 3, 146, 169, 191, 94, 180, 77, 75, 179, 122, 130, 175, 136, 107, 176, 85, 107, 176, 80, 112, 180, 118, 99, 178, 133, 114, 178, 137, 111, 19};


//: #B391FF
Byte kStr_enemyRemoteForData[] = {77, 7, 10, 9, 46, 172, 53, 30, 204, 25, 56, 41, 47, 39, 60, 60, 55};
...
@interface ListViewController ()<UITableViewDataSource,UITableViewDelegate,CipherColor,MaxRecent>
...
- (instancetype)initWithName:(WithReply *)dataSource {
    ...
    if (self) {
        ...
        NSString *kNIMTeamListDataTeamMembersChanged = StringFromInstitutionalizedData(kStr_backgroundTitle);
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(boardMember:) name:kNIMTeamListDataTeamMembersChanged object:nil];
    }
    //: return self;
    return self;
}
...
- (void)viewDidLoad {
    ...
    [PhotoManager towardExtra:[NSString stringWithFormat:StringFromInstitutionalizedData(kStr_disableKillerName)] tool:dict examine:NO titleResponseSuccess:^(id responseObject) {
        //: NSDictionary *resultDict = (NSDictionary *)responseObject;
        NSDictionary *resultDict = (NSDictionary *)responseObject;
        //: NSString *code = [resultDict newStringValueForKey:@"code"];
        NSString *code = [resultDict with:StringFromInstitutionalizedData(kStr_wantName)];
        //: if (code.integerValue <= 0) {
        if (code.integerValue <= 0) {
            //: NSDictionary *data = [resultDict valueObjectForKey:@"data"];
            NSDictionary *data = [resultDict view:StringFromInstitutionalizedData(kStr_itTenderName)];
...
- (void)orientation {
    ...
    [backButton setImage:[UIImage imageNamed:StringFromInstitutionalizedData(kStr_occasionallyContent)] forState:(UIControlStateNormal)];
    ...
    labtitle.text = [ItemManager sessionInformation:StringFromInstitutionalizedData(kStr_hotelTitle)];
    ...
    submitButton.frame = CGRectMake([[UIScreen mainScreen] bounds].size.width-15-40, [UIDevice clear]+8, 32, 32);
    //: submitButton.backgroundColor = [UIColor colorWithHexString:@"#B391FF"];
    submitButton.backgroundColor = [UIColor withMax:StringFromInstitutionalizedData(kStr_enemyRemoteForData)];
    //: submitButton.layer.cornerRadius = 16;
    submitButton.layer.cornerRadius = 16;
    //: [submitButton setImage:[UIImage imageNamed:@"ic_group_addmember"] forState:(UIControlStateNormal)];
    [submitButton setImage:[UIImage imageNamed:StringFromInstitutionalizedData(kStr_budgeTitle)] forState:(UIControlStateNormal)];
    ...
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(15, (44.0f + [UIDevice clear])+15, [[UIScreen mainScreen] bounds].size.width-30, [[UIScreen mainScreen] bounds].size.height-(44.0f + [UIDevice clear])) style:UITableViewStyleGrouped];
    ...
    //: } else {
        ...
        [[NIMSDK sharedSDK].teamManager updateMuteState:ismute
            ...
            if (!error) {
                ...
                msg = [ItemManager sessionInformation:StringFromInstitutionalizedData(kStr_egoWithName)];
            //: }else{
            }else{
                //: msg = [FFFLanguageManager getTextWithKey:@"group_info_activity_op_failed"];
                msg = [ItemManager sessionInformation:StringFromInstitutionalizedData(kStr_thoroughName)];
            }
...
- (void)compartmenting:(NSString *)uid
{
    ...
    [self.teamListManager inputText:@[uid] lozenge:^(NSError * _Nonnull error, NSString * _Nonnull msg) {
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ParView *cell = [ParView behindMedia:tableView];
    //: cell.delegate = self;
    cell.delegate = self;
    ...
    MarginThanInfo *usrInfo = [[Afterwards blue] put:uId button:nil];
    ...
    cell.subtitleLabel.text = [ItemManager sessionInformation:StringFromInstitutionalizedData(kStr_yardHotelData)];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSString *canMemberInfo = [_teamSettingConfig with:StringFromInstitutionalizedData(kStr_chainData)];
...
- (void)frameEffectCheck:(NSArray *)selectedContacts{
    ...
    [self to:selectedContacts tableView:nil];
}

//: - (void)didInviteUsers:(NSArray<NSString *> *)userIds
- (void)to:(NSArray<NSString *> *)userIds
            //: completion:(dispatch_block_t)completion {
            tableView:(dispatch_block_t)completion {
    ...
    NSMutableDictionary *info = [NSMutableDictionary dictionary];
    //: info[@"attach"] = @"扩展消息";
    info[StringFromInstitutionalizedData(kStr_nabTitle)] = StringFromInstitutionalizedData(kStr_twentiethTitle);
    //: switch (self.teamListManager.team.type) {
    switch (self.teamListManager.team.type) {
        //: case NIMTeamTypeNormal:
        case NIMTeamTypeNormal:
            //: info[@"postscript"] = @"邀请你加入讨论组".nim_localized;
            info[StringFromInstitutionalizedData(kStr_dryTitle)] = StringFromInstitutionalizedData(kStr_illnessName).textKey;
            //: break;
            break;
        //: case NIMTeamTypeAdvanced:
        case NIMTeamTypeAdvanced:
            //: info[@"postscript"] = @"邀请你加入高级群".nim_localized;
            info[StringFromInstitutionalizedData(kStr_dryTitle)] = StringFromInstitutionalizedData(kStr_effectivePatientValue).textKey;
            //: break;
            break;
        //: case NIMTeamTypeSuper:
        case NIMTeamTypeSuper:
            //: info[@"postscript"] = @"邀请你加入超大群".nim_localized;
            info[StringFromInstitutionalizedData(kStr_dryTitle)] = StringFromInstitutionalizedData(kStr_mobileText).textKey;
            //: break;
            break;
        //: default:
        default:
...
Byte * InstitutionalizedDataToCache(Byte *data) {
    int formatObject = data[0];
    int inherent = data[1];
    Byte careful = data[2];
    int journeyHotel = data[3];
    if (!formatObject) return data + journeyHotel;
    for (int i = journeyHotel; i < journeyHotel + inherent; i++) {
        int value = data[i] + careful;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
    ...
    data[0] = 0;
    data[journeyHotel + inherent] = 0;
    return data + journeyHotel;
}

NSString *StringFromInstitutionalizedData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)InstitutionalizedDataToCache(data)];
}
