...
NSString *StringFromThinkData(Byte *data);


//: 1080x1920
Byte kStr_brokenCheekData[] = {44, 9, 11, 224, 254, 182, 233, 201, 119, 60, 236, 48, 50, 57, 49, 120, 48, 56, 48, 49, 146};


//: 480x640
Byte kStr_leasedName[] = {9, 7, 10, 164, 62, 58, 186, 233, 214, 200, 48, 52, 54, 120, 48, 56, 52, 221};


//: 720x1280
Byte kStr_refugeeRedInquiryData[] = {96, 8, 7, 63, 58, 121, 173, 48, 56, 50, 49, 120, 48, 50, 55, 167};


//: face_rect
Byte kStr_hamData[] = {68, 9, 11, 208, 187, 121, 158, 234, 35, 196, 45, 116, 99, 101, 114, 95, 101, 99, 97, 102, 13};


//: resolution:\n%dx%d\nfps: %d\nrender time:\n%.0fms
Byte kStr_tractionData[] = {96, 45, 6, 75, 221, 30, 115, 109, 102, 48, 46, 37, 10, 58, 101, 109, 105, 116, 32, 114, 101, 100, 110, 101, 114, 10, 100, 37, 32, 58, 115, 112, 102, 10, 100, 37, 120, 100, 37, 10, 58, 110, 111, 105, 116, 117, 108, 111, 115, 101, 114, 55};
...
@interface GenerateNotice ()<FURenderKitDelegate, FUCaptureCameraDataSource>
...
- (instancetype)init {
    ...
    if (self) {
        ...
        if ((self.necessaryAIModelTypes & FUAIModelTypeHuman) && self.loadAIModelAutomatically) {
            ...
            [LanguagePin high:FUHumanSegmentationModeCPUCommon];
        }
        ...
        [[LanguagePin titleUser] disableTo];
    }
    //: return self;
    return self;
}
...
- (void)model:(FUGLDisplayView *)renderView {
    // 需要音频轨道
...
- (void)fellow:(AVCaptureSessionPreset)preset untilLibrary:(void (^)(void))handler {
...
- (void)getStarted:(BOOL)front array:(void (^)(void))handler {
    //: if (![FURenderKit shareRenderKit].captureCamera) {
    if (![FURenderKit shareRenderKit].captureCamera) {
        //: return;
        return;
...
- (void)container:(FUCaptureCameraFocusMode)mode {
    //: if (![FURenderKit shareRenderKit].captureCamera) {
    if (![FURenderKit shareRenderKit].captureCamera) {
        //: return;
        return;
...
- (void)container {
    ...
    if ([LanguagePin text] && [FURenderKit shareRenderKit].captureCamera) {
        //: center = [self trackedFaceCenter];
        center = [self messageBy];
...
- (CGPoint)messageBy {
    ...
    [FUAIKit getFaceInfo:0 name:StringFromThinkData(kStr_hamData) pret:rect number:4];
    //: CGFloat minX = rect[0];
    CGFloat minX = rect[0];
    //: CGFloat minY = rect[1];
    CGFloat minY = rect[1];
...
- (void)renderKitWillRenderFromRenderInput:(FURenderInput *)renderInput {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(caned:)]) {
        //: [self.delegate renderWillInputPixelBuffer:renderInput.pixelBuffer];
        [self.delegate caned:renderInput.pixelBuffer];
    }
...
- (void)renderKitDidRenderToOutput:(FURenderOutput *)renderOutput {
    ...
    if (endTime - lastCalculateTime >= 1) {
        ...
        NSString *debugString = [NSString stringWithFormat:StringFromThinkData(kStr_tractionData), width, height, rate, currentCalculateTime * 1000 / rate];
        //: if (self.delegate && [self.delegate respondsToSelector:@selector(renderDidOutputDebugInformations:)]) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(invited:)]) {
            //: [self.delegate renderDidOutputDebugInformations:debugString];
            [self.delegate invited:debugString];
        }
        // 恢复计算数据
    ...
    if (self.detectingParts != FUDetectingPartsNone) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(counting:)]) {
            //: [self.delegate renderShouldCheckDetectingStatus:self.detectingParts];
            [self.delegate counting:self.detectingParts];
        }
    }
}
...
- (void)setMaxFaceNumber:(NSInteger)maxFaceNumber {
    ...
    [LanguagePin setRange:maxFaceNumber];
}
...
- (void)setMaxHumanNumber:(NSInteger)maxHumanNumber {
    ...
    [LanguagePin setShowIconView:maxHumanNumber];
}
...
- (NSArray<NSString *> *)presetTitles {
    ...
    for (AVCaptureSessionPreset preset in self.presets) {
        ...
        if (preset == AVCaptureSessionPreset640x480) {
            ...
            [titles addObject:StringFromThinkData(kStr_leasedName)];
        //: } else if (preset == AVCaptureSessionPreset1280x720) {
        } else if (preset == AVCaptureSessionPreset1280x720) {
            //: [titles addObject:@"720x1280"];
            [titles addObject:StringFromThinkData(kStr_refugeeRedInquiryData)];
        //: } else if (preset == AVCaptureSessionPreset1920x1080) {
        } else if (preset == AVCaptureSessionPreset1920x1080) {
            //: [titles addObject:@"1080x1920"];
            [titles addObject:StringFromThinkData(kStr_brokenCheekData)];
        }
    }
    //: return [titles copy];
    return [titles copy];
...
Byte * ThinkDataToCache(Byte *data) {
    int purchase = data[0];
    int restoreBrand = data[1];
    int why = data[2];
    if (!purchase) return data + why;
    for (int i = 0; i < restoreBrand / 2; i++) {
        int begin = why + i;
        int end = why + restoreBrand - i - 1;
        Byte temp = data[begin];
        data[begin] = data[end];
        data[end] = temp;
    }
    data[0] = 0;
    data[why + restoreBrand] = 0;
    return data + why;
}

NSString *StringFromThinkData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)ThinkDataToCache(data)];
}  
