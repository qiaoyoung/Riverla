...
#import "PassePartoutView.h"

//: @interface FUSegmentBar ()
@interface PassePartoutView ()
...
@implementation CipherConfigurations
...
static NSString * const let_makeValue = @"FUSegmentCellIdentifier";

//: @interface FUSegmentBar ()<UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout>
@interface PassePartoutView ()<UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout>

//: @property (nonatomic, strong) UICollectionView *collectionView;
@property (nonatomic, strong) UICollectionView *collectionView;

//: @property (nonatomic, strong) FUSegmentBarConfigurations *configuration;
@property (nonatomic, strong) CipherConfigurations *configuration;
...
@implementation PassePartoutView
...
- (instancetype)initWithConfiguration:(CGRect)frame first:(NSArray<NSString *> *)titles nonsolidColourConfiguration:(CipherConfigurations *)configuration {
    ...
    if (self) {
        ...
        if (!self.configuration) {
            ...
            self.configuration = [[CipherConfigurations alloc] init];
        }
...
- (void)componentImage:(NSInteger)index {
    ...
    if (self.selectedIndex == index) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(straightaway:at:)]) {
            //: [self.delegate segmentBar:self didSelectItemAtIndex:index];
            [self.delegate straightaway:self at:index];
        }
        //: return;
        return;
    }
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    TextInfoReusableView *cell = [collectionView dequeueReusableCellWithReuseIdentifier:let_makeValue forIndexPath:indexPath];
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(lipFullMoonBunglePress:colorShow:)]) {
        //: if ([self.delegate segmentBar:self shouldDisableItemAtIndex:indexPath.item]) {
        if ([self.delegate lipFullMoonBunglePress:self colorShow:indexPath.item]) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(straightaway:at:)]) {
        ...
        [self.delegate straightaway:self at:indexPath.item];
    }
}
...
- (BOOL)collectionView:(UICollectionView *)collectionView shouldSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(replyBar:ting:)]) {
        //: return [self.delegate segmentBar:self shouldSelectItemAtIndex:indexPath.item];
        return [self.delegate replyBar:self ting:indexPath.item];
    }
    //: return YES;
    return YES;
}
...
- (UICollectionView *)collectionView {
    ...
    if (!_collectionView) {
        ...
        [_collectionView registerClass:[TextInfoReusableView class] forCellWithReuseIdentifier:let_makeValue];
    }
...
@implementation TextInfoReusableView
