...
+ (void)image:(NSString *)tipsString notice:(NSTimeInterval)delay {
    ...
    [self replyCell:tipsString info:delay delay:FUTipHUDPositionTop];
}
...
+ (void)replyCell:(NSString *)tipsString info:(NSTimeInterval)delay delay:(FUTipHUDPosition)position {
    ...
    [views enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([obj isMemberOfClass:[RecentLayerView class]]) {
    ...
    __block RecentLayerView *tipLabel = [[RecentLayerView alloc] initWithAssumedTitleInsets:CGRectZero background:UIEdgeInsetsMake(8, 20, 8, 20)];
