...
NSString *StringFromDocumentaryData(Byte *data);


//: #612CF9
Byte kStr_tornSharedData[] = {46, 7, 79, 8, 57, 22, 195, 12, 114, 133, 128, 129, 146, 149, 136, 99};


//: #222222
Byte kStr_burningSubmitText[] = {32, 7, 30, 11, 16, 88, 8, 8, 100, 56, 33, 65, 80, 80, 80, 80, 80, 80, 81};


//: back_arrow_bl
Byte kStr_confirmDeviceValue[] = {54, 13, 16, 14, 173, 37, 40, 174, 229, 169, 121, 87, 224, 226, 114, 113, 115, 123, 111, 113, 130, 130, 127, 135, 111, 114, 124, 90};


//: #875FFA
Byte kStr_equalContent[] = {24, 7, 65, 10, 156, 52, 231, 240, 200, 18, 100, 121, 120, 118, 135, 135, 130, 205};
...
#import "UIViewController+StopAdditions.h"

//: @implementation UIViewController (KIAdditions)
@implementation UIViewController (StopAdditions)
...
- (void)from {
    ...
    [self index:@selector(samplingSend)];
}

//: - (void)showCustomBackButton:(SEL)selector {
- (void)index:(SEL)selector {
    //: [self setNavLeftItem:selector image:[UIImage imageNamed:@"back_arrow_bl"] imageH:nil];
    [self tongueTing:selector videoOff:[UIImage imageNamed:StringFromDocumentaryData(kStr_confirmDeviceValue)] event:nil];

}

//: - (void)setNavLeftItem:(SEL)selector image:(UIImage *)image imageH:(UIImage *)imageH {
- (void)tongueTing:(SEL)selector videoOff:(UIImage *)image event:(UIImage *)imageH {
...
- (void)voiceWith:(SEL)selector set:(UIImage *)image lightIn:(UIImage *)imageH {
    //: UIButton *backButton = [UIButton buttonWithType:UIButtonTypeCustom];
    UIButton *backButton = [UIButton buttonWithType:UIButtonTypeCustom];
    //: [backButton setImage:image forState:UIControlStateNormal];
    [backButton setImage:image forState:UIControlStateNormal];
...
-(void)dateText:(SEL)selector blueDisable:(NSString *)title gesture:(UIColor *)color
{
    //: UIButton *backButton = [UIButton buttonWithType:UIButtonTypeCustom];
    UIButton *backButton = [UIButton buttonWithType:UIButtonTypeCustom];
...
-(void)rightItemTextTeeAchromaticColour:(SEL)selector color:(NSString *)title blooper:(UIColor *)color
{
    //: UIButton *backButton = [UIButton buttonWithType:UIButtonTypeCustom];
    UIButton *backButton = [UIButton buttonWithType:UIButtonTypeCustom];
    //: [backButton setTitle:title forState:UIControlStateNormal];
...
- (void)setTitle:(NSString *)title {
    ...
    [titleLabel setTextColor:[UIColor withMax:StringFromDocumentaryData(kStr_burningSubmitText)]];
...
- (void)setTheShared:(UIColor *)color{
    //: UILabel *titleLabel = (UILabel *)self.navigationItem.titleView;
    UILabel *titleLabel = (UILabel *)self.navigationItem.titleView;
    //: if (!titleLabel) {
    if (!titleLabel) {
...
- (void)pinSet:(NSString *)title set:(UIColor *)color{
    //: UILabel *titleLabel = (UILabel *)self.navigationItem.titleView;
    UILabel *titleLabel = (UILabel *)self.navigationItem.titleView;
    //: if (!titleLabel) {
    if (!titleLabel) {
...
- (void)pop {
    ...
    //: } else {
        ...
        [self.view bugOutWithinCompletion:^(BOOL finished) {
...
- (BOOL)smallInfo:(UIView *)view hide:(BOOL)hide{
    //: UIView *line = [self findHairlineImageViewUnder:view];
    UIView *line = [self ting:view];
...
- (void)setConfirm:(UIColor *)color{
    ...
    [self.navigationController.navigationBar setBackgroundImage:[MarginMax rallyUpwards:[UIColor withMax:StringFromDocumentaryData(kStr_equalContent)] blackType:[UIColor withMax:StringFromDocumentaryData(kStr_tornSharedData)] bringIn:SNLinearGradientDirectionLevel] forBarMetrics:UIBarMetricsDefault];
...
- (void)past{
    ...
    [self.navigationController.navigationBar setBackgroundImage:[self lineupColor:[UIColor clearColor]] forBarMetrics:UIBarMetricsDefault];
    //: self.navigationController.navigationBar.shadowImage = [UIImage new];
    self.navigationController.navigationBar.shadowImage = [UIImage new];
    //: self.navigationController.navigationBar.translucent = YES;
    self.navigationController.navigationBar.translucent = YES;
...
- (void)setIndicatorAlpha:(CGFloat)alpha{
...
- (UIImage *)lineupColor:(UIColor *)color{
...
Byte * DocumentaryDataToCache(Byte *data) {
    int grant = data[0];
    int roadHero = data[1];
    Byte masterKey = data[2];
    int logBlue = data[3];
    if (!grant) return data + logBlue;
    for (int i = logBlue; i < logBlue + roadHero; i++) {
        int value = data[i] - masterKey;
        if (value < 0) {
            value += 256;
        }
        data[i] = value;
    ...
    data[0] = 0;
    data[logBlue + roadHero] = 0;
    return data + logBlue;
}

NSString *StringFromDocumentaryData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)DocumentaryDataToCache(data)];
}
