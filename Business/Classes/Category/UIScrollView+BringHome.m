...
NSString *StringFromHeaterData(Byte *data);


//: frame
Byte kStr_taleLemmaData[] = {66, 5, 4, 86, 101, 109, 97, 114, 102, 5};


//: 下拉加载...
Byte kStr_lidName[] = {31, 15, 7, 98, 248, 79, 245, 46, 46, 46, 189, 189, 232, 160, 138, 229, 137, 139, 230, 139, 184, 228, 149};


//: 松开刷新...
Byte kStr_putData[] = {50, 15, 10, 77, 241, 175, 37, 50, 37, 111, 46, 46, 46, 176, 150, 230, 183, 136, 229, 128, 188, 229, 190, 157, 230, 166};


//: contentOffset
Byte kStr_myTingText[] = {90, 13, 8, 153, 240, 73, 99, 84, 116, 101, 115, 102, 102, 79, 116, 110, 101, 116, 110, 111, 99, 21};


//: Pull to refresh...
Byte kStr_historicallyAllImproveValue[] = {2, 18, 12, 192, 47, 249, 122, 150, 131, 72, 39, 19, 46, 46, 46, 104, 115, 101, 114, 102, 101, 114, 32, 111, 116, 32, 108, 108, 117, 80, 224};


//: SVPullToRefreshView
Byte kStr_assistValue[] = {52, 19, 4, 48, 119, 101, 105, 86, 104, 115, 101, 114, 102, 101, 82, 111, 84, 108, 108, 117, 80, 86, 83, 193};


//: 加载中...
Byte kStr_favorableName[] = {78, 12, 3, 46, 46, 46, 173, 184, 228, 189, 189, 232, 160, 138, 229, 83};


//: contentSize
Byte kStr_residencyText[] = {44, 11, 11, 114, 213, 181, 138, 62, 88, 46, 207, 101, 122, 105, 83, 116, 110, 101, 116, 110, 111, 99, 198};

// __DEBUG__
// __CLOSE_PRINT__
//
// UIScrollView+BringHome.m
//
...
#import "UIScrollView+BringHome.h"
//: #import <objc/runtime.h>
#import <objc/runtime.h>

//: static CGFloat const NTESPullToRefreshViewHeight = 60;
static CGFloat const let_windowContent = 60;
...
- (void)setEmptyBoom:(UIEdgeInsets)insets;
//: - (void)rotateArrow:(float)degrees hide:(BOOL)hide;
- (void)save:(float)degrees sumCrop:(BOOL)hide;
...
#pragma mark - UIScrollView (BringHome)


//: static char UIScrollViewPullToRefreshView;
static char let_inviteValue;

//: @implementation UIScrollView (NTESPullToRefresh)
@implementation UIScrollView (BringHome)
...
- (void)signal:(void (^)(void))actionHandler sheetRestorePosition:(NTESPullToRefreshPosition)position {
    ...
    if(!self.pullToRefreshView) {
        ...
        FactoidView *view = [[FactoidView alloc] initWithFrame:CGRectMake(0, yOrigin, self.bounds.size.width, let_windowContent)];
...
- (void)indicator:(void (^)(void))actionHandler {
    ...
    [self signal:actionHandler sheetRestorePosition:NTESPullToRefreshPositionTop];
}
...
- (void)start {
    ...
    [self.pullToRefreshView segment];
}

//: - (void)setPullToRefreshView:(NTESPullToRefreshView *)pullToRefreshView {
- (void)setPullToRefreshView:(FactoidView *)pullToRefreshView {
    //: [self willChangeValueForKey:@"SVPullToRefreshView"];
    [self willChangeValueForKey:StringFromHeaterData(kStr_assistValue)];
    //: objc_setAssociatedObject(self, &UIScrollViewPullToRefreshView,
    objc_setAssociatedObject(self, &let_inviteValue,
                             //: pullToRefreshView,
                             pullToRefreshView,
                             //: OBJC_ASSOCIATION_ASSIGN);
                             OBJC_ASSOCIATION_ASSIGN);
    //: [self didChangeValueForKey:@"SVPullToRefreshView"];
    [self didChangeValueForKey:StringFromHeaterData(kStr_assistValue)];
}

//: - (NTESPullToRefreshView *)pullToRefreshView {
- (FactoidView *)pullToRefreshView {
    //: return objc_getAssociatedObject(self, &UIScrollViewPullToRefreshView);
    return objc_getAssociatedObject(self, &let_inviteValue);
}
...
- (void)setShowsPullToRefresh:(BOOL)showsPullToRefresh {
    ...
    if(!showsPullToRefresh) {
        ...
        if (self.pullToRefreshView.isObserving) {
            ...
            [self removeObserver:self.pullToRefreshView forKeyPath:StringFromHeaterData(kStr_myTingText)];
            //: [self removeObserver:self.pullToRefreshView forKeyPath:@"contentSize"];
            [self removeObserver:self.pullToRefreshView forKeyPath:StringFromHeaterData(kStr_residencyText)];
            //: [self removeObserver:self.pullToRefreshView forKeyPath:@"frame"];
            [self removeObserver:self.pullToRefreshView forKeyPath:StringFromHeaterData(kStr_taleLemmaData)];
            //: [self.pullToRefreshView resetScrollViewContentInset];
            [self.pullToRefreshView messageRed];
    ...
    //: else {
        ...
        if (!self.pullToRefreshView.isObserving) {
            ...
            [self addObserver:self.pullToRefreshView forKeyPath:StringFromHeaterData(kStr_myTingText) options:NSKeyValueObservingOptionNew context:nil];
            //: [self addObserver:self.pullToRefreshView forKeyPath:@"contentSize" options:NSKeyValueObservingOptionNew context:nil];
            [self addObserver:self.pullToRefreshView forKeyPath:StringFromHeaterData(kStr_residencyText) options:NSKeyValueObservingOptionNew context:nil];
            //: [self addObserver:self.pullToRefreshView forKeyPath:@"frame" options:NSKeyValueObservingOptionNew context:nil];
            [self addObserver:self.pullToRefreshView forKeyPath:StringFromHeaterData(kStr_taleLemmaData) options:NSKeyValueObservingOptionNew context:nil];
            ...
            self.pullToRefreshView.frame = CGRectMake(0, yOrigin, self.bounds.size.width, let_windowContent);
        }
    }
}
...
#pragma mark - BringHome
//: @implementation NTESPullToRefreshView
@implementation FactoidView
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if(self = [super initWithFrame:frame]) {
        ...
        self.titles = [NSMutableArray arrayWithObjects:NSLocalizedString(StringFromHeaterData(kStr_lidName),),
                       //: NSLocalizedString(@"松开刷新...",),
                       NSLocalizedString(StringFromHeaterData(kStr_putData),),
                       //: NSLocalizedString(@"加载中...",),
                       NSLocalizedString(StringFromHeaterData(kStr_favorableName),),
                       //: nil];
                       nil];
    ...
    if (self.superview && newSuperview == nil) {
        ...
        if (scrollView.showsPullToRefresh) {
            ...
            if (self.isObserving) {
                ...
                [scrollView removeObserver:self forKeyPath:StringFromHeaterData(kStr_myTingText)];
                //: [scrollView removeObserver:self forKeyPath:@"contentSize"];
                [scrollView removeObserver:self forKeyPath:StringFromHeaterData(kStr_residencyText)];
                //: [scrollView removeObserver:self forKeyPath:@"frame"];
                [scrollView removeObserver:self forKeyPath:StringFromHeaterData(kStr_taleLemmaData)];
                //: self.isObserving = NO;
                self.isObserving = NO;
            }
        }
            ...
            case NTESPullToRefreshStateStopped:
                ...
                switch (self.position) {
                    ...
                    case NTESPullToRefreshPositionTop:
                        ...
                        [self save:0 sumCrop:NO];
                        //: break;
                        break;
                    //: case NTESPullToRefreshPositionBottom:
                    case NTESPullToRefreshPositionBottom:
                        //: [self rotateArrow:(float)3.14159265358979323846264338327950288 hide:NO];
                        [self save:(float)3.14159265358979323846264338327950288 sumCrop:NO];
                        //: break;
                        break;
                }
                //: break;
            ...
            case NTESPullToRefreshStateLoading:
                ...
                switch (self.position) {
                    ...
                    case NTESPullToRefreshPositionTop:
                        ...
                        [self save:0 sumCrop:YES];
                        //: break;
                        break;
                    //: case NTESPullToRefreshPositionBottom:
                    case NTESPullToRefreshPositionBottom:
                        //: [self rotateArrow:(float)3.14159265358979323846264338327950288 hide:YES];
                        [self save:(float)3.14159265358979323846264338327950288 sumCrop:YES];
                        //: break;
                        break;
                }
                //: break;
...
- (void)messageRed {
    ...
    [self setEmptyBoom:currentInsets];
}
...
- (void)setEmptyBoom:(UIEdgeInsets)contentInset {
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    if([keyPath isEqualToString:StringFromHeaterData(kStr_myTingText)])
        //: [self scrollViewDidScroll:[[change valueForKey:NSKeyValueChangeNewKey] CGPointValue]];
        [self scroll:[[change valueForKey:NSKeyValueChangeNewKey] CGPointValue]];
    //: else if([keyPath isEqualToString:@"contentSize"]) {
    else if([keyPath isEqualToString:StringFromHeaterData(kStr_residencyText)]) {
        //: [self layoutSubviews];
        [self layoutSubviews];
        ...
        self.frame = CGRectMake(0, yOrigin, self.bounds.size.width, let_windowContent);
    }
    //: else if([keyPath isEqualToString:@"frame"])
    else if([keyPath isEqualToString:StringFromHeaterData(kStr_taleLemmaData)])
        //: [self layoutSubviews];
        [self layoutSubviews];
...
- (void)scroll:(CGPoint)contentOffset {
...
- (FaceView *)arrow {
    ...
    if(!_arrow) {
        ...
        _arrow = [[FaceView alloc]initWithFrame:CGRectMake(0, self.bounds.size.height-54, 22, 48)];
...
- (UILabel *)titleLabel {
    ...
    if(!_titleLabel) {
        ...
        _titleLabel.text = NSLocalizedString(StringFromHeaterData(kStr_historicallyAllImproveValue),);
...
Byte * HeaterDataToCache(Byte *data) {
    int queryJury = data[0];
    int curiousDaughter = data[1];
    int spaceOption = data[2];
    if (!queryJury) return data + spaceOption;
    for (int i = 0; i < curiousDaughter / 2; i++) {
        int begin = spaceOption + i;
        int end = spaceOption + curiousDaughter - i - 1;
        Byte temp = data[begin];
        data[begin] = data[end];
        data[end] = temp;
    }
    data[0] = 0;
    data[spaceOption + curiousDaughter] = 0;
    return data + spaceOption;
}

NSString *StringFromHeaterData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)HeaterDataToCache(data)];
}  
