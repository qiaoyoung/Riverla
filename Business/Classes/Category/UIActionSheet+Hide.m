...
static char kLet_tapMagnitudeValue;


//: @implementation UIActionSheet (NTESBlock)
@implementation UIActionSheet (Hide)
...
- (void)with: (UIView *)view stochastic: (ActionSheetBlock)block
{
    ...
    objc_setAssociatedObject(self,&kLet_tapMagnitudeValue,block,OBJC_ASSOCIATION_COPY);
    ...
    else
    {
        ...
        else
        {
            ...
            [self performSelector:@selector(exampled:)
                       //: withObject:view
                       withObject:view
                       //: afterDelay:1];
                       afterDelay:1];
...
- (void)actionSheet:(UIActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex
{
    ...
    ActionSheetBlock block = [objc_getAssociatedObject(self, &kLet_tapMagnitudeValue) copy];
    //: objc_setAssociatedObject(self,&kUIActionSheetBlockAddress,nil,OBJC_ASSOCIATION_COPY);
    objc_setAssociatedObject(self,&kLet_tapMagnitudeValue,nil,OBJC_ASSOCIATION_COPY);
