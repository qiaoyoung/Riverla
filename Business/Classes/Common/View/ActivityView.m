...
+ (instancetype)segmentTip:(NSString *)badgeValue{
    ...
    ActivityView *instance = [[ActivityView alloc] init];
    //: instance.frame = [instance frameWithStr:badgeValue];
    instance.frame = [instance along:badgeValue];
...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([[self badgeValue] length]) {
        ...
        [self elect:rect showTo:context];
    //: }else{
    }else{
        //: [self drawWithOutContent:rect context:context];
        [self color:rect screenReply:context];
    }
...
- (void)setBadgeValue:(NSString *)badgeValue {
    ...
    self.frame = [self along:badgeValue];
...
- (CGRect)along:(NSString *)badgeValue{
...
- (void)elect:(CGRect)rect showTo:(CGContextRef)context{
...
- (void)color:(CGRect)rect screenReply:(CGContextRef)context{
    //: CGRect bodyFrame = self.bounds;
    CGRect bodyFrame = self.bounds;
