...
NSString *StringFromRapData(Byte *data);


//: UITabBarButton
Byte kStr_surpriseContent[] = {91, 14, 10, 86, 84, 252, 234, 131, 94, 49, 110, 111, 116, 116, 117, 66, 114, 97, 66, 98, 97, 84, 73, 85, 207};

// __DEBUG__
// __CLOSE_PRINT__
//
//  GeorgiaHomeBoyIcon.m
//  NIM
//
...
@implementation GeorgiaHomeBoyIcon
//: static inline BOOL
static inline BOOL
...
+ (void)load
{
    ...
    _dispatch_once(&onceToken, ^{
        ...
        if (@available(iOS 12.1, *)) { // 解决12.1 的UITabbar 位置显示异常
            ...
            OverrideImplementation(NSClassFromString(StringFromRapData(kStr_surpriseContent)), @selector(setFrame:), ^id(__unsafe_unretained Class originClass, SEL originCMD, IMP originIMP) {
...
- (void)animateTransition:(id<UIViewControllerContextTransitioning>)transitionContext
{
    ...
    switch (self.currentOpearation) {
        ...
        case UINavigationControllerOperationPop:
            ...
            [self label:transitionContext];
            //: break;
            break;
        //: case UINavigationControllerOperationPush:
        case UINavigationControllerOperationPush:
            //: [self pushAnimation:transitionContext];
            [self recentWithAnimation:transitionContext];
            //: break;
            break;
        //: default:
        default:
...
- (void)recentWithAnimation:(id<UIViewControllerContextTransitioning>)transitionContext
{
...
- (void)label:(id<UIViewControllerContextTransitioning>)transitionContext
{
    //: UIViewController *toViewController = [transitionContext viewControllerForKey:UITransitionContextToViewControllerKey];
    UIViewController *toViewController = [transitionContext viewControllerForKey:UITransitionContextToViewControllerKey];
    //: UIViewController *fromViewController = [transitionContext viewControllerForKey:UITransitionContextFromViewControllerKey];
    ...
    CGFloat snapshootHeight = [UIDevice table] + fromViewController.navigationController.navigationBar.height;
    ...
    UITabBar *tabbar = [SoapBarController with].tabBar;
    //: UIView *containerView = [transitionContext containerView];
    UIView *containerView = [transitionContext containerView];
    ...
    //: } completion:^(BOOL finished) {
        ...
        [[SoapBarController with].view addSubview:tabbar];
...
- (void)trackAt
{
    ...
    if ([self.delegate respondsToSelector:@selector(originColor:)]) {
        //: [self.delegate animationWillStart:self];
        [self.delegate originColor:self];
    }
}
...
- (void)by
{
    ...
    if ([self.delegate respondsToSelector:@selector(sounded:)]) {
        //: [self.delegate animationDidEnd:self];
        [self.delegate sounded:self];
    }
}
...
Byte * RapDataToCache(Byte *data) {
    int ice = data[0];
    int intervalegratedBishop = data[1];
    int radiogram = data[2];
    if (!ice) return data + radiogram;
    for (int i = 0; i < intervalegratedBishop / 2; i++) {
        int begin = radiogram + i;
        int end = radiogram + intervalegratedBishop - i - 1;
        Byte temp = data[begin];
        data[begin] = data[end];
        data[end] = temp;
    }
    data[0] = 0;
    data[radiogram + intervalegratedBishop] = 0;
    return data + radiogram;
}

NSString *StringFromRapData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)RapDataToCache(data)];
}  
