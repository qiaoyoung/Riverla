...
//: before_yesterday
Byte kStr_todayName[] = {89, 16, 68, 5, 117, 166, 169, 170, 179, 182, 169, 163, 189, 169, 183, 184, 169, 182, 168, 165, 189, 233};


//: %d小时前
Byte kStr_groupTitle[] = {31, 11, 1, 11, 51, 185, 248, 142, 41, 175, 123, 38, 101, 230, 177, 144, 231, 152, 183, 230, 138, 142, 217};


//: %d分钟前
Byte kStr_flueName[] = {45, 11, 46, 11, 206, 38, 209, 173, 148, 17, 201, 83, 146, 19, 182, 180, 23, 192, 205, 19, 183, 187, 200};


//: %d小时前更新
Byte kStr_apologyCellValue[] = {66, 17, 37, 8, 60, 29, 15, 112, 74, 137, 10, 213, 180, 11, 188, 219, 10, 174, 178, 11, 192, 217, 11, 187, 213, 160};


//: yyyy-MM-dd
Byte kStr_mumTitle[] = {4, 10, 42, 12, 177, 249, 83, 210, 222, 219, 228, 233, 163, 163, 163, 163, 87, 119, 119, 87, 142, 142, 145};


//: %d分钟前更新
Byte kStr_progressText[] = {4, 17, 17, 11, 199, 129, 195, 211, 30, 80, 86, 54, 117, 246, 153, 151, 250, 163, 176, 246, 154, 158, 247, 172, 197, 247, 167, 193, 2};


//: yyyy.MM.dd更新
Byte kStr_recentBooName[] = {9, 16, 59, 12, 13, 180, 109, 39, 101, 138, 197, 235, 180, 180, 180, 180, 105, 136, 136, 105, 159, 159, 33, 214, 239, 33, 209, 235, 248};


//: yesterday
Byte kStr_geneValue[] = {57, 9, 47, 6, 138, 31, 168, 148, 162, 163, 148, 161, 147, 144, 168, 62};
...
@implementation NSDate (StopAdditions)
...
+ (NSString *)clear:(NSDate*)date {
    ...
    return [dateFormatter distanceMax:date];
}
...
+ (NSString *)month:(NSDate*)date {
    ...
    return [dateFormatter head:date];
}
...
+ (NSString *)year:(NSDate*)date {
    ...
    return [dateFormatter someMessageFlush:date];
}

//: + (NSCalendar *)calendar:(NSDate*)date {
+ (NSCalendar *)doDetectSize:(NSDate*)date {
...
+ (int)tutorial:(NSDate*)date {
    ...
    return (int)[[NSDate doDetectSize:date] rangeOfUnit:NSCalendarUnitDay
...
+ (int)model:(NSDate*)date {
    ...
    return (int)[[NSDate doDetectSize:date] rangeOfUnit:NSCalendarUnitWeekOfMonth
                                             //: inUnit:NSCalendarUnitMonth
                                             inUnit:NSCalendarUnitMonth
                                            //: forDate:date].length;
                                            forDate:date].length;
...
+ (NSString*)select:(NSString*)oldDateString
    ...
    NSDate *oldDate = [NSDate item:oldDateString infoFormatter:oldFormatterString];

    //: NSString *returnString = [NSDate getStringFromDate:oldDate dateFormatter:newFormatterString];
    NSString *returnString = [NSDate addFormatter:oldDate successOf:newFormatterString];
...
- (NSString *)itemView
{
    ...
    if ([components day] >= 3) {
        ...
        return [NSDate addFormatter:self successOf:StringFromUpgradeData(kStr_mumTitle)];
    //: } else if ([components day] >= 2) {
    } else if ([components day] >= 2) {
        //: return [FFFLanguageManager getTextWithKey:@"before_yesterday"];
        return [ItemManager sessionInformation:StringFromUpgradeData(kStr_todayName)];
    //: } else if ([components day] >= 1) {
    } else if ([components day] >= 1) {
        //: return [FFFLanguageManager getTextWithKey:@"yesterday"];
        return [ItemManager sessionInformation:StringFromUpgradeData(kStr_geneValue)];
    //: } else if ([components hour] > 0) {
    } else if ([components hour] > 0) {
        //: return [NSString stringWithFormat:@"%d小时前", (int)[components hour]];
        return [NSString stringWithFormat:StringFromUpgradeData(kStr_groupTitle), (int)[components hour]];
    //: } else if ([components minute] > 0) {
    } else if ([components minute] > 0) {
        //: return [NSString stringWithFormat:@"%d分钟前", (int)[components minute]];
        return [NSString stringWithFormat:StringFromUpgradeData(kStr_flueName), (int)[components minute]];
...
- (NSString *)size
{
    ...
    if ([components day] >= 1) {
        ...
        return [NSDate addFormatter:self successOf:StringFromUpgradeData(kStr_recentBooName)];
    //: } else if ([components hour] > 0) {
    } else if ([components hour] > 0) {
        //: return [NSString stringWithFormat:@"%d小时前更新", (int)[components hour]];
        return [NSString stringWithFormat:StringFromUpgradeData(kStr_apologyCellValue), (int)[components hour]];
    //: } else if ([components minute] > 0) {
    } else if ([components minute] > 0) {
        //: return [NSString stringWithFormat:@"%d分钟前更新", (int)[components minute]];
        return [NSString stringWithFormat:StringFromUpgradeData(kStr_progressText), (int)[components minute]];
    //: }else {
    }else {
        //: return @"刚刚";
        return @"刚刚";
...
Byte * UpgradeDataToCache(Byte *data) {
    int tornButtonRestore = data[0];
    int usufructuary = data[1];
    Byte brig = data[2];
    int highStatusColor = data[3];
    if (!tornButtonRestore) return data + highStatusColor;
    for (int i = highStatusColor; i < highStatusColor + usufructuary; i++) {
        int value = data[i] - brig;
        if (value < 0) {
            value += 256;
        }
        data[i] = value;
    ...
    data[0] = 0;
    data[highStatusColor + usufructuary] = 0;
    return data + highStatusColor;
}
