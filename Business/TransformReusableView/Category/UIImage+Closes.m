...
@implementation UIImage (Closes)
//: + (UIImage *)fetchImage:(NSString *)imageNameOrPath{
+ (UIImage *)periodicTable:(NSString *)imageNameOrPath{
...
- (UIImage *)max
{
    ...
    CGFloat pixels = [[RangeRecent viewOf] option];
    //: UIImage * image = [self imageForUpload:pixels];
    UIImage * image = [self readValue:pixels];
...
- (UIImage *)readValue: (CGFloat)suggestPixels
{
        ...
        (width / height > kMaxRatio || height / width > kMaxRatio))
    {
        ...
        return [self touch:kMaxPixels];
    }
    //: else
    else
    {
        //: return [self scaleWithMaxPixels:suggestPixels];
        return [self touch:suggestPixels];
    }
}

//: - (UIImage *)scaleWithMaxPixels: (CGFloat)maxPixels
- (UIImage *)touch: (CGFloat)maxPixels
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
    ...
    return [self tableCell:CGSizeMake(newSizeWidth, newSizeHeight)];
}

//内缩放，一条变等于最长边，另外一条小于等于最长边
//: - (UIImage *)scaleToSize:(CGSize)newSize
- (UIImage *)tableCell:(CGSize)newSize
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
