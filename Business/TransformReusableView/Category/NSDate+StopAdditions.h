...
@interface NSDate (StopAdditions)
...
+ (int)tutorial:(NSDate*)date;

/*获取当前月有多少周*/
//: + (int)weeksOfMonth:(NSDate*)date;
+ (int)model:(NSDate*)date;

/*是否在某个日期之前*/
//: - (BOOL)isBefore:(NSDate *)date;
- (BOOL)recent:(NSDate *)date;

/*是否在某个日期之后*/
//: - (BOOL)isAfter:(NSDate *)date;
- (BOOL)pressedAfter:(NSDate *)date;

/*是否在某个时间闭区间之间*/
//: - (BOOL)isBetween:(NSDate *)startDate and:(NSDate *)endDate;
- (BOOL)at:(NSDate *)startDate title:(NSDate *)endDate;
...
+ (BOOL)team:(NSDate*)date01 grievousBodilyHarm:(NSUInteger)days;
...
+ (BOOL)pin:(NSString*)date1String01 house:(NSString*)date1String02 success:(NSString*)formatter01 size:(NSString*)formatter02;

//: + (BOOL)isString:(NSString*)date1String01 earlierThanDate:(NSDate*)date02 formatter01:(NSString*)formatter01;
+ (BOOL)offCenter:(NSString*)date1String01 image:(NSDate*)date02 button:(NSString*)formatter01;

//: + (NSDate *)dateForTodayInClock:(NSInteger)clock;
+ (NSDate *)titleClock:(NSInteger)clock;
