...
NSString *const kLet_windowValue = @"online_state";
...
@interface ImageColor()<NIMEventSubscribeManagerDelegate,NIMConversationManagerDelegate,NIMLoginManagerDelegate,NIMUserManagerDelegate>
...
+ (instancetype)sharedUserAdministratorName
{
    ...
    static ImageColor *instance;
    //: static dispatch_once_t onceToken;
    static dispatch_once_t onceToken;
    //: _dispatch_once(&onceToken, ^{
    _dispatch_once(&onceToken, ^{
        //: instance = [[NTESSubscribeManager alloc] init];
        instance = [[ImageColor alloc] init];
    //: });
    });
    //: return instance;
    return instance;
...
- (void)trackDoing:(NSString *)userId
{
    ...
    NIMSubscribeRequest *request = [self progressEnable];
...
- (void)end
{
    ...
    NSDictionary *ext = @{
                            ...
                            kLet_versionName : @([RangeRecent viewOf].scrub),
                            //: NTESSubscribeOnlineState : @(NTESOnlineStateNormal), 
                            kLet_windowValue : @(NTESOnlineStateNormal), //移动端永远在线
                          //: };
                          };
    //: [event setExt:[ext nimkit_jsonString]];
    [event setExt:[ext infoTitle]];
...
- (void)custom
{
    ...
    [_subscribeIds addObjectsFromArray:self.change.allObjects];
    //: [_subscribeIds addObjectsFromArray:self.contactUserIds.allObjects];
    [_subscribeIds addObjectsFromArray:self.need.allObjects];

    //: [self subscribeNextMembers:_subscribeIds.allObjects];
    [self pinDeviceDate:_subscribeIds.allObjects];
}

//: - (void)subscribeNextMembers:(NSArray *)ids
- (void)pinDeviceDate:(NSArray *)ids
{
    //: if (!ids.count) {
    if (!ids.count) {
        //: return;
    ...
    [[NIMSDK sharedSDK].subscribeManager subscribeEvent:request completion:^(NSError * _Nullable error, NSArray * _Nullable failedPublishers) {
        ...
        if (members.count) {
            ...
            [weakSelf pinDeviceDate:members];
        }
    //: }];
    }];
}
...
- (void)onFriendChanged:(NIMUser *)user
{
    ...
    if (!isMyFriend && ![self.change containsObject:user.userId]) {
              ...
              totalUnreadCount:(NSInteger)totalUnreadCount
{
    ...
    if (recentSession.session.sessionType == NIMSessionTypeP2P && ![self.need containsObject:recentSession.session.sessionId]) {
...
- (NIMSubscribeRequest *)progressEnable
{
