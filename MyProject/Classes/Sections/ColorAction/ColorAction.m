...
NSString *const let_barName = @"online_state";
...
@interface ColorAction()<NIMEventSubscribeManagerDelegate,NIMConversationManagerDelegate,NIMLoginManagerDelegate,NIMUserManagerDelegate>
...
- (void)nim:(NSString *)userId
{
    ...
    NIMSubscribeRequest *request = [self bringForth];
...
- (void)recordingLabel
{
    ...
    NSDictionary *ext = @{
                            ...
                            let_myValue : @([PictureShowDevice performance].labelType),
                            //: NTESSubscribeOnlineState : @(NTESOnlineStateNormal), 
                            let_barName : @(NTESOnlineStateNormal), //移动端永远在线
                          //: };
                          };
    //: [event setExt:[ext nimkit_jsonString]];
    [event setExt:[ext nimkit]];
...
- (void)direction
{
    ...
    [_subscribeIds addObjectsFromArray:self.titlePath.allObjects];
    //: [_subscribeIds addObjectsFromArray:self.contactUserIds.allObjects];
    [_subscribeIds addObjectsFromArray:self.tinkle.allObjects];

    //: [self subscribeNextMembers:_subscribeIds.allObjects];
    [self nextSubscribeWith:_subscribeIds.allObjects];
}

//: - (void)subscribeNextMembers:(NSArray *)ids
- (void)nextSubscribeWith:(NSArray *)ids
{
    //: if (!ids.count) {
    if (!ids.count) {
        //: return;
    ...
    [[NIMSDK sharedSDK].subscribeManager subscribeEvent:request completion:^(NSError * _Nullable error, NSArray * _Nullable failedPublishers) {
        ...
        if (members.count) {
            ...
            [weakSelf nextSubscribeWith:members];
        }
    //: }];
    }];
}
...
- (void)onFriendChanged:(NIMUser *)user
{
    ...
    if (!isMyFriend && ![self.titlePath containsObject:user.userId]) {
              ...
              totalUnreadCount:(NSInteger)totalUnreadCount
{
    ...
    if (recentSession.session.sessionType == NIMSessionTypeP2P && ![self.tinkle containsObject:recentSession.session.sessionId]) {
...
- (NIMSubscribeRequest *)bringForth
{
