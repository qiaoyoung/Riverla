...
@interface YieldByCuttingEdgeView : UIView {
//: @private
@private
    //: CGRect _cropRect;
    CGRect _cropRect;
...
@implementation MentalImageView
...
- (void)setFrame:(CGRect)frame {
    ...
    if (__CGSizeEqualToSize(_cropSize, CGSizeZero)) {
        ...
        [self setLoad:CGSizeMake(100, 100)];
    }
}
...
- (YieldByCuttingEdgeView *)maskView {
    //: if (_maskView == nil) {
    if (_maskView == nil) {
        //: _maskView = [[KICropImageMaskView alloc] init];
        _maskView = [[YieldByCuttingEdgeView alloc] init];
...
- (void)setTipPress:(UIImage *)image {
    ...
    [[self thoughtImageWith] setImage:_image];
...
- (void)sharedTo {
    ...
    [[self thoughtImageWith] setFrame:CGRectMake(0, 0, width, height)];
...
- (void)setLoad:(CGSize)size {
    ...
    [(YieldByCuttingEdgeView *)[self maskView] setInfo:_cropSize];
    ...
    [[self off] setContentInset:_imageInset];

    //: [[self scrollView] setContentOffset:CGPointMake(0, 0)];
    [[self off] setContentOffset:CGPointMake(0, 0)];
}
...
- (UIImage *)subColor {
    ...
    image = [image rubric:resizeWidth rangeCan:resizeheight];
...
@implementation YieldByCuttingEdgeView
