...
NSString *StringFromArcherData(ArcherData *data);


//: login_bg
ArcherData kStr_consecutiveData = (ArcherData){13, (Byte []){97, 98, 106, 100, 99, 82, 111, 106, 157}, 8, false, 85, 247, 80};


//: estimatedProgress
ArcherData kStr_onRedValue = (ArcherData){60, (Byte []){89, 79, 72, 85, 81, 93, 72, 89, 88, 108, 78, 83, 91, 78, 89, 79, 79, 46}, 17, false, 39, 119, 38};


//: #B391FF
ArcherData kStr_richValue = (ArcherData){204, (Byte []){239, 142, 255, 245, 253, 138, 138, 180}, 7, false, 104, 240, 131};


//: type
ArcherData kStr_hateTitle = (ArcherData){51, (Byte []){71, 74, 67, 86, 184}, 4, false, 140, 233, 137};


//: #009ADC
ArcherData kStr_memoValue = (ArcherData){14, (Byte []){45, 62, 62, 55, 79, 74, 77, 33}, 7, false, 186, 106, 133};


//: jsCallOC
ArcherData kStr_conIndicateData = (ArcherData){21, (Byte []){127, 102, 86, 116, 121, 121, 90, 86, 242}, 8, false, 137, 211, 215};


//: html
ArcherData kStr_comeData = (ArcherData){105, (Byte []){1, 29, 4, 5, 207}, 4, false, 3, 129, 100};


//: Agreed
ArcherData kStr_absolutelyOvenValue = (ArcherData){234, (Byte []){171, 141, 152, 143, 143, 142, 63}, 6, false, 231, 189, 119};


//: back_arrow_bl
ArcherData kStr_crossData = (ArcherData){162, (Byte []){192, 195, 193, 201, 253, 195, 208, 208, 205, 213, 253, 192, 206, 205}, 13, false, 68, 88, 196};


//: Privacy Agreement 20200602
ArcherData kStr_stationPartData = (ArcherData){151, (Byte []){199, 229, 254, 225, 246, 244, 238, 183, 214, 240, 229, 242, 242, 250, 242, 249, 227, 183, 165, 167, 165, 167, 167, 161, 167, 165, 201}, 26, false, 19, 194, 99};

// __DEBUG__
// __CLOSE_PRINT__
//
//  GinMillViewController.m
//  Panda
//
...
#import "GinMillViewController.h"
//: #import <WebKit/WebKit.h>
#import <WebKit/WebKit.h>

//: @interface HMWebViewController () <WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler> {
@interface GinMillViewController () <WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler> {
...
@implementation GinMillViewController
...
//    [self showCustomBackButton];
    //    self.view.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"login_bg"]];
        ...
        bg.image = [UIImage imageNamed:StringFromArcherData(&kStr_consecutiveData)];
    ...
    [self.closeBtn setImage:[UIImage imageNamed:StringFromArcherData(&kStr_crossData)] forState:(UIControlStateNormal)];
    ...
    [self.agreeBtn setTitle:StringFromArcherData(&kStr_absolutelyOvenValue) forState:UIControlStateNormal];

//    self.agreeBtn.layer.masksToBounds = YES;
    //: self.agreeBtn.backgroundColor = [UIColor colorWithHexString:@"#B391FF"];
    self.agreeBtn.backgroundColor = [UIColor isView:StringFromArcherData(&kStr_richValue)];
    //: self.agreeBtn.layer.cornerRadius = 10;
    self.agreeBtn.layer.cornerRadius = 10;
    //: self.agreeBtn.layer.shadowColor = [UIColor colorWithHexString:@"#009ADC"].CGColor;
    self.agreeBtn.layer.shadowColor = [UIColor isView:StringFromArcherData(&kStr_memoValue)].CGColor;
    ...
    [self.agreeBtn addTarget:self action:@selector(tipWill) forControlEvents:UIControlEventTouchUpInside];
...
- (void)dealloc {
    ...
    [_webView removeObserver:self forKeyPath:StringFromArcherData(&kStr_onRedValue)];
}
...
//    self.view.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"login_bg"]];
    ...
    [_webView addObserver:self forKeyPath:StringFromArcherData(&kStr_onRedValue) options:NSKeyValueObservingOptionNew context:nil];


    //: [self jsConfig];
    [self messageOpenConfig];
...
- (void)at{
    ...
    //: }else{
        ...
        NSString *htmlPath = [[NSBundle mainBundle] pathForResource:StringFromArcherData(&kStr_stationPartData) ofType:StringFromArcherData(&kStr_comeData)];
...
- (void)bubbleTitle:(WKWebView *)webView navigation:(WKNavigation *)navigation{
    //: self.progressView.hidden = YES;
    self.progressView.hidden = YES;
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context {
    ...
    if (object == self.webView && [keyPath isEqualToString:StringFromArcherData(&kStr_onRedValue)]) {
...
//: - (void)jsConfig{
- (void)messageOpenConfig{

    // js配置
    //: WKUserContentController *userContentController = _webView.configuration.userContentController;
    WKUserContentController *userContentController = _webView.configuration.userContentController;
    //: [userContentController addScriptMessageHandler:self name:@"jsCallOC"];
    [userContentController addScriptMessageHandler:self name:StringFromArcherData(&kStr_conIndicateData)];
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message{
    ...
    if ([message.name isEqualToString:StringFromArcherData(&kStr_conIndicateData)]) {
        //: [self dealJsData:message.body];
        [self backWithData:message.body];
    }
...
- (void)backWithData:(NSDictionary *)jsDict{
    ...
    NSInteger type = [[jsDict speedy:StringFromArcherData(&kStr_hateTitle)] integerValue];
...
Byte *ArcherDataToByte(ArcherData *data) {
    if (data->exceptionCotta) return data->admit;
    for (int i = 0; i < data->explodeLid; i++) {
        data->admit[i] ^= data->quickOrigin;
    }
    data->admit[data->explodeLid] = 0;
    data->exceptionCotta = true;
	if (data->explodeLid >= 3) {
		data->neighborly = data->admit[0];
		data->strengthen = data->admit[1];
		data->consultant = data->admit[2];
	}
    return data->admit;
}

NSString *StringFromArcherData(ArcherData *data) {
    return [NSString stringWithUTF8String:(char *)ArcherDataToByte(data)];
}
