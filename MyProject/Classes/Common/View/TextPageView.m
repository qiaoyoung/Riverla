...
@interface CustomPictureScrollView : UIScrollView

//: @end
@end

//: @interface NTESPageView ()
@interface TextPageView ()
{
...
- (void)should
{
    ...
    if (_scrollView == nil)
    {
        ...
        _scrollView = [[CustomPictureScrollView alloc]initWithFrame:self.bounds];
...
- (NSInteger)by:(NSInteger)value pictureMax:(NSInteger)min pop:(NSInteger)max{
    //: if (max < min) {
    if (max < min) {
        //: max = min;
        max = min;
...
- (void)elitePage:(NSInteger)currentPage
{
    ...
    NSInteger first = [self by:currentPage - 1 pictureMax:0 pop:count - 1];
    //: NSInteger last = [self pageInBound:currentPage + 1 min:0 max:count - 1];
    NSInteger last = [self by:currentPage + 1 pictureMax:0 pop:count - 1];
...
- (void)message
{
    ...
    [self elitePage:_currentPage];
    //: [self raisePageIndexChangedDelegate];
    [self performanceTing];
    //: [self setNeedsLayout];
    [self setNeedsLayout];
}
...
@implementation CustomPictureScrollView
//fix: swipe to delete in a UITableView which is embeded in a UIScrollView
//: - (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldRequireFailureOfGestureRecognizer:(nonnull UIGestureRecognizer *)otherGestureRecognizer
- (BOOL)loadValue:(UIGestureRecognizer *)gestureRecognizer image:(nonnull UIGestureRecognizer *)otherGestureRecognizer
{
