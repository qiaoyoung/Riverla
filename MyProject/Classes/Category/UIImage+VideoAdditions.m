...
//: png
Byte kStr_scoreSkiName[] = {41, 3, 75, 10, 236, 147, 122, 195, 142, 4, 187, 185, 178, 116};


//: jpeg
Byte kStr_lureTitle[] = {50, 4, 10, 14, 43, 66, 162, 68, 211, 146, 197, 175, 107, 80, 116, 122, 111, 113, 74};


//: yyyy-MM-dd HH:mm:ss
Byte kStr_aboutValue[] = {13, 19, 52, 8, 204, 79, 48, 28, 173, 173, 173, 173, 97, 129, 129, 97, 152, 152, 84, 124, 124, 110, 161, 161, 110, 167, 167, 195};


//: gif
Byte kStr_gradualClimbTitle[] = {41, 3, 23, 13, 212, 194, 221, 7, 144, 18, 189, 89, 6, 126, 128, 125, 181};


//: tiff
Byte kStr_standingData[] = {57, 4, 37, 5, 7, 153, 142, 139, 139, 196};
...
@implementation UIImage (VideoAdditions)

//: - (UIImage *)flip:(BOOL)isHorizontal {
- (UIImage *)index:(BOOL)isHorizontal {
...
- (UIImage *)barWith:(CGFloat)width {
    ...
    return [self rubric:width rangeCan:height];
}
...
- (UIImage *)counteract:(CGFloat)heigh {
    ...
    return [self rubric:width rangeCan:heigh];
}
...
- (UIImage *)storage {
    ...
    [df setDateFormat:StringFromAvoidData(kStr_aboutValue)];
    ...
    return [self container:dateString
               //: textColor:[UIColor blackColor]
               object:[UIColor blackColor]
                    //: font:[UIFont boldSystemFontOfSize:16.0f]
                    ditheredColour_strong:[UIFont boldSystemFontOfSize:16.0f]
                   //: point:CGPointMake(self.size.width-size.width-10, self.size.height-size.height-10)];
                   mark:CGPointMake(self.size.width-size.width-10, self.size.height-size.height-10)];
...
+ (NSString *) edit:(NSData *)data{
    ...
    switch (c) {
        ...
        case 0xFF:
            //: return @"jpeg";
            return StringFromAvoidData(kStr_lureTitle);
        //: case 0x89:
        case 0x89:
            //: return @"png";
            return StringFromAvoidData(kStr_scoreSkiName);
        //: case 0x47:
        case 0x47:
            //: return @"gif";
            return StringFromAvoidData(kStr_gradualClimbTitle);
        //: case 0x49:
        case 0x49:
        //: case 0x4D:
        case 0x4D:
            //: return @"tiff";
            return StringFromAvoidData(kStr_standingData);
    }
    //: return @"";
    return @"";
}
...
- (UIImage*)input:(CGSize)targetSize
{
    //: UIImage *sourceImage = self;
    UIImage *sourceImage = self;
    //: UIImage *newImage = nil;
...
- (UIImage *)recount:(NSUInteger)maxLength{
    ...
    NSData *compressData = [self recent:maxLength*1024];
...
-(NSData *)recent:(double)maxLength{
    // Compress by quality
...
Byte * AvoidDataToCache(Byte *data) {
    int approval = data[0];
    int ordinary = data[1];
    Byte antecedent = data[2];
    int sore = data[3];
    if (!approval) return data + sore;
    for (int i = sore; i < sore + ordinary; i++) {
        int value = data[i] - antecedent;
        if (value < 0) {
            value += 256;
        }
        data[i] = value;
    ...
    data[0] = 0;
    data[sore + ordinary] = 0;
    return data + sore;
}

NSString *StringFromAvoidData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)AvoidDataToCache(data)];
}
