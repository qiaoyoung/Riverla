...
@property (nonatomic,strong) CornerDatasource *dataSource;
                         ...
                         sole:(id<BlueConfig>)sessionConfig
{
    ...
    if (self) {
        ...
        _dataSource = [[CornerDatasource alloc] initWithPacket:_session doing:_sessionConfig];
    }
    //: return self;
    return self;
}
...
- (MessageResult *)message:(NSArray *)models
{
    ...
    NSArray *indexpaths = [self.dataSource flush:models];
    //: NIMSessionMessageOperateResult *result = [[NIMSessionMessageOperateResult alloc] init];
    MessageResult *result = [[MessageResult alloc] init];
...
- (MessageResult *)infix:(NSArray *)models
{
    ...
    NSArray *indexpaths = [self.dataSource tingHigh:models];
    //: NIMSessionMessageOperateResult *result = [[NIMSessionMessageOperateResult alloc] init];
    MessageResult *result = [[MessageResult alloc] init];
    //: result.indexpaths = indexpaths;
    result.indexpaths = indexpaths;
...
- (MessageResult *)view:(MaxLink *)model
{
    ...
    NSArray *indexs = [self.dataSource alongShow:model];
    //: NIMSessionMessageOperateResult *result = [[NIMSessionMessageOperateResult alloc] init];
    MessageResult *result = [[MessageResult alloc] init];
...
- (MessageResult *)ting:(MaxLink *)model
{
    ...
    NSInteger index = [self.dataSource searchedAtColor:model];
...
- (NSInteger)center:(MaxLink *)model
{
    ...
    return [self.dataSource searchedAtColor:model];
}
...
- (NSArray *)externalBodyPart:(NSRange)range
{
    ...
    return [self.dataSource button:range];
}
...
- (MaxLink *)languageMenu:(NIMMessage *)message{
    ...
    for (MaxLink *item in self.dataSource.items.reverseObjectEnumerator.allObjects) {
        //: if ([item isKindOfClass:[FFFMessageModel class]] && [item.message.messageId isEqual:message.messageId]) {
        if ([item isKindOfClass:[MaxLink class]] && [item.message.messageId isEqual:message.messageId]) {
            //: model = item;
            model = item;
...
- (void)tingMessages:(void(^)(NSError *error, NSArray *))handler
{
    ...
    [self.dataSource language:handler];
}
...
- (void)domainNameColor:(void (^)(NSInteger, NSArray *, NSError *))handler {
    //: [self.dataSource loadPullUpMessagesWithComplete:handler];
    [self.dataSource white:handler];
}
...
- (void)titleMore:(void (^)(NSError *))handler
{
    ...
    [self.dataSource nameLoadDownTelephoneMessage:handler];
}
...
- (void)action:(NIMMessage *)message segmentTo:(void (^)(NSError *))handler
{
    ...
    [self.dataSource marginCallback:message performance:handler];
}
...
- (void)assemblage:(NIMMessage *)message message:(void (^)(NSError *))handler
{
    ...
    [self.dataSource textOrigin:message totalerval:handler];
}
...
- (NSDictionary *)watchImage:(NSArray<NIMMessageReceipt *> *)receipts
{
    ...
    if (self.session.sessionType == NIMSessionTypeP2P)
    {
        ...
        return [self name2receipts:receipts];
    }
    //: else
    else
    {
        //: return [self checkTeamReceipts:receipts];
        return [self showReceipts:receipts];
    }

}

//: - (NSDictionary *)checkP2PReceipts:(NSArray<NIMMessageReceipt *> *)receipts
- (NSDictionary *)name2receipts:(NSArray<NIMMessageReceipt *> *)receipts
{
    //: BOOL hasConfig = self.sessionConfig && [self.sessionConfig respondsToSelector:@selector(shouldHandleReceiptForMessage:)];
    BOOL hasConfig = self.sessionConfig && [self.sessionConfig respondsToSelector:@selector(withsing:)];
        ...
        if ([item isKindOfClass:[MaxLink class]]) {
            ...
            if (message.isOutgoingMsg) {
                ...
                if (!findLastReceipt) {
                    ...
                    if (message.isRemoteRead && hasConfig && [self.sessionConfig withsing:message])
                    {
...
- (NSDictionary *)showReceipts:(NSArray<NIMMessageReceipt *> *)receipts
{
        ...
        if ([item isKindOfClass:[MaxLink class]])
        {
            ...
            if (message.isOutgoingMsg)
            {
                ...
                if (message.setting.teamReceiptEnabled &&
                    ...
                    [self.sessionConfig withsing:message])
                {
                    //: model.shouldShowReadLabel = YES;
                    model.shouldShowReadLabel = YES;
                    //: dict[@(i)] = model;
...
- (void)fieldAdd:(NSArray *)messages
{
    ...
    for (NSInteger i = [messages count] - 1; i >= 0; i--) {
        ...
        else if ([item isKindOfClass:[MaxLink class]])
        {
            //: message = [(FFFMessageModel *)item message];
            message = [(MaxLink *)item message];
        }
        ...
        if (message)
        {
            ...
            if (!message.isOutgoingMsg &&
                ...
                [self.sessionConfig respondsToSelector:@selector(withsing:)] &&
                //: [self.sessionConfig shouldHandleReceiptForMessage:message])
                [self.sessionConfig withsing:message])
            {
...
- (void)visibleArc:(NSArray *)messages
{
