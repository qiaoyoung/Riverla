 ...
 *************************简要说明************************
 ...
 [ByAlert configMainWindow:self.window];
 
 
 Max 使用方法
 
 [ByAlert alert].cofing.XXXXX.XXXXX.LeeShow();
 
 Recent 使用方法
 
 [ByAlert actionSheet].cofing.XXXXX.XXXXX.LeeShow();
...
@interface ByAlert: NSObject

/** 初始化 */

//: + (nonnull LEEAlertConfig *)alert;
+ (nonnull TextBaseConfig *)colorDown;

//: + (nonnull LEEActionSheetConfig *)actionsheet;
+ (nonnull ViewPictureBaseConfig *)key;

/** 获取Alert窗口 */
//: + (nonnull LEEAlertWindow *)getAlertWindow;
+ (nonnull ShowWindow *)alertQuick;
...
+ (void)shared:(NSString *)identifier anDisable:(BOOL)force view:(void (^ _Nullable)(void))completionBlock;
...
+ (void)completionned:(void (^ _Nullable)(void))completionBlock;

//: @end
@end

//: @interface LEEBaseConfigModel: NSObject
@interface WithLayer: NSObject
...
@interface WithLayer(Presentation)

/** 设置 层级 -> 格式: .LeePresentation([NameInput windowLevel:1000] / [NameInput viewController:xxxx]) */
...
@interface EvaluateLength: NSObject
...
@interface DoingAction: NSObject
...
By CornerRadiiZero(void);

//: CornerRadii CornerRadiiNull(void);
By CornerRadiiNull(void);
