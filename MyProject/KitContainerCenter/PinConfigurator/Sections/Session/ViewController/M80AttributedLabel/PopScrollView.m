...
static NSString* const let_toData = @"\u2026";
...
- (NSAttributedString *)attributed:(NSString *)text
{
    //: if ([text length])
    if ([text length])
    {
...
- (NSAttributedString *)unwished:(NSMutableAttributedString *)drawString
{
    //: if (drawString)
    if (drawString)
    {
...
- (NSMutableAttributedString *)info:(NSAttributedString *)attributedString {
    // 匹配 [表情]
    ...
    if (attrM.length > 0) {
        ...
        NSArray<NSTextCheckingResult *> *emoticonResults = [[PopScrollView related] matchesInString:attrM.string options:kNilOptions range:attrM.yy_rangeOfAll];
        ...
        [emoticonResults enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(NSTextCheckingResult * _Nonnull emo, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            CellEmoticon *emoticon = [[AppManager administrator] doName:emoString];

            //: UIImage *image = [UIImage nim_emoticonInKit:emoticon.filename];
            UIImage *image = [UIImage disable:emoticon.filename];
...
- (NSMutableAttributedString*)underBackground:(NSMutableAttributedString *)wholeText {
...
- (void)setText:(NSString *)text
{
    ...
    NSAttributedString *attributedText = [self attributed:text];
...
- (void)setAttributedText:(NSAttributedString *)attributedText
{
    ...
    NSAttributedString *attString = [self unwished:[[NSMutableAttributedString alloc] initWithAttributedString:attributedText]];
    //: _attributedString = [self needHightText:[self transformEmojiDescToEomjiImageWithAttributedString:attString]];
    _attributedString = [self underBackground:[self info:attString]];
...
- (void)telegram:(NSString *)text
{
    //: NSAttributedString *attributedText = [self attributedString:text];
    NSAttributedString *attributedText = [self attributed:text];
    //: [self appendAttributedText:attributedText];
    [self upwards:attributedText];
}

//: - (void)appendAttributedText:(NSAttributedString *)attributedText
- (void)upwards:(NSAttributedString *)attributedText
{
