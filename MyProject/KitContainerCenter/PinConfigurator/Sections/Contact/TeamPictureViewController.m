...
NSString *StringFromNightlongData(Byte *data);


//: no_friend
Byte kStr_mannerTitle[] = {91, 9, 5, 10, 28, 39, 144, 126, 23, 25, 115, 116, 100, 107, 119, 110, 106, 115, 105, 238};


//: select_contact
Byte kStr_quarterData[] = {13, 14, 23, 6, 151, 169, 138, 124, 131, 124, 122, 139, 118, 122, 134, 133, 139, 120, 122, 139, 24};


//: icon_checkbox_selected
Byte kStr_quickContent[] = {91, 22, 87, 6, 192, 98, 192, 186, 198, 197, 182, 186, 191, 188, 186, 194, 185, 198, 207, 182, 202, 188, 195, 188, 186, 203, 188, 187, 90};


//: ic_choose_w
Byte kStr_debtContent[] = {11, 11, 2, 5, 172, 107, 101, 97, 101, 106, 113, 113, 117, 103, 97, 121, 123};


//: SelectContactCellID
Byte kStr_agencyPinTitle[] = {12, 19, 28, 8, 135, 126, 36, 59, 111, 129, 136, 129, 127, 144, 95, 139, 138, 144, 125, 127, 144, 95, 129, 136, 136, 101, 96, 88};


//: No friends，please add friends
Byte kStr_titleBrokenValue[] = {59, 31, 7, 4, 85, 118, 39, 109, 121, 112, 108, 117, 107, 122, 246, 195, 147, 119, 115, 108, 104, 122, 108, 39, 104, 107, 107, 39, 109, 121, 112, 108, 117, 107, 122, 63};


//: #B391FF
Byte kStr_showScreenAgainContent[] = {4, 7, 29, 12, 127, 194, 178, 104, 68, 57, 135, 20, 64, 95, 80, 86, 78, 99, 99, 72};


//: #999999
Byte kStr_voterVesselRegularData[] = {92, 7, 33, 14, 179, 75, 107, 36, 81, 118, 89, 145, 137, 83, 68, 90, 90, 90, 90, 90, 90, 43};


//: ic_close_b
Byte kStr_illustrateFaintValue[] = {73, 10, 61, 10, 249, 175, 167, 2, 14, 148, 166, 160, 156, 160, 169, 172, 176, 162, 156, 159, 253};


//: main_activity_choose_least_one
Byte kStr_neutralName[] = {98, 30, 89, 10, 118, 133, 203, 200, 90, 187, 198, 186, 194, 199, 184, 186, 188, 205, 194, 207, 194, 205, 210, 184, 188, 193, 200, 200, 204, 190, 184, 197, 190, 186, 204, 205, 184, 200, 199, 190, 47};
...
#import "SetTitlePanoramicView.h"
...
@interface TeamPictureViewController ()<UITableViewDataSource, UITableViewDelegate, NumbereractionHouseDelegateVideo>{
...
@property(nonatomic, strong) SetTitlePanoramicView *setGroupnameView;
...
@property (strong, nonatomic) DeviceView *selectIndicatorView;
...
- (void)viewDidLoad
{
    ...
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, (44.0f + [UIDevice table]), [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height-(44.0f + [UIDevice table])) style:UITableViewStylePlain];
...
- (void)apposeColor
{
    ...
    [backButton setImage:[UIImage imageNamed:StringFromNightlongData(kStr_illustrateFaintValue)] forState:(UIControlStateNormal)];
    ...
    labtitle.text = [TeamManager sizeKey:StringFromNightlongData(kStr_quarterData)];
    ...
    submitButton.frame = CGRectMake([[UIScreen mainScreen] bounds].size.width-15-48, [UIDevice table]+10, 48, 28);
    //: submitButton.backgroundColor = [UIColor colorWithHexString:@"#B391FF"];
    submitButton.backgroundColor = [UIColor isView:StringFromNightlongData(kStr_showScreenAgainContent)];
    //: submitButton.layer.cornerRadius = 14;
    submitButton.layer.cornerRadius = 14;
    //: [submitButton setImage:[UIImage imageNamed:@"ic_choose_w"] forState:(UIControlStateNormal)];
    [submitButton setImage:[UIImage imageNamed:StringFromNightlongData(kStr_debtContent)] forState:(UIControlStateNormal)];
    //: [submitButton addTarget:self action:@selector(onDoneBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    [submitButton addTarget:self action:@selector(refreshClick:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)remote
{
    ...
    if(_selectecContacts.count > 0){
        ...
        [btn setTitle:self.provider forState:UIControlStateNormal];
        //: [btn setImage:[UIImage imageNamed:@"icon_checkbox_selected"] forState:UIControlStateNormal];
        [btn setImage:[UIImage imageNamed:StringFromNightlongData(kStr_quickContent)] forState:UIControlStateNormal];
...
- (void)streetSmart
{
    ...
    if(_selectecContacts.count > 0){
        ...
        self.tableView.frame = CGRectMake(0, (44.0f + [UIDevice table])+64, [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height-(44.0f + [UIDevice table])-64);
    //: }else{
    }else{
        ...
        self.tableView.frame = CGRectMake(0, (44.0f + [UIDevice table]), [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height-(44.0f + [UIDevice table]));
    }
}
...
- (void)setConfig:(id<LinkConfig>)config{
    //: _config = config;
    _config = config;
    //: if ([config respondsToSelector:@selector(maxSelectedNum)]) {
    if ([config respondsToSelector:@selector(showColor)]) {
        //: _maxSelectCount = [config maxSelectedNum];
        _maxSelectCount = [config showColor];
...
- (void)translateMove:(id)sender {
    ...
    [self dismissViewControllerAnimated:YES completion:^() {
        ...
        if([_delegate respondsToSelector:@selector(containerTap)]) {
            //: [_delegate didCancelledSelect];
            [_delegate containerTap];
        }
    //: }];
    }];
}
...
- (IBAction)refreshClick:(id)sender {
    ...
    if (_selectecContacts.count == 0) {
        ...
        [SVProgressHUD rangeMessage:[TeamManager sizeKey:StringFromNightlongData(kStr_neutralName)]];
        //: return;
        return;
    }
             ...
             try{} @finally{} __typeof__(self) self = __weak_self__;
                ...
                if (_selectecContacts.count) {
                    ...
                    if ([self.delegate respondsToSelector:@selector(ting:)]) {
                        ...
                        [self.delegate ting:_selectecContacts];
                    }

                    //: FFFGroupAvatarViewController *vc = [[FFFGroupAvatarViewController alloc]init];
                    ScoopViewController *vc = [[ScoopViewController alloc]init];
...
- (void)cherryRed{
    ...
    [self.config of:^(NSDictionary *contentDic, NSArray *titles) {
    ...
    if ([self.config respondsToSelector:@selector(kibbutznikContent)])
    {
        //: _selectecContacts = [[self.config alreadySelectedMemberId] mutableCopy];
        _selectecContacts = [[self.config kibbutznikContent] mutableCopy];
    }
    ...
    for (NSString *selectId in _selectecContacts) {
        ...
        info = [self.config will:selectId];
        //: [self.selectIndicatorView.pickedView addMemberInfo:info];
        [self.selectIndicatorView.pickedView member:info];
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    id<BoardMemberLayer> contactItem = arr[indexPath.row];

    //: FFFContactDataCell *cell = [tableView dequeueReusableCellWithIdentifier:@"SelectContactCellID"];
    LanguageViewCell *cell = [tableView dequeueReusableCellWithIdentifier:StringFromNightlongData(kStr_agencyPinTitle)];
    //: if (cell == nil) {
    if (cell == nil) {
        //: cell = [[FFFContactDataCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"SelectContactCellID"];
        cell = [[LanguageViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:StringFromNightlongData(kStr_agencyPinTitle)];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    info = [self.config will:memberId];
...
- (DeviceView *)selectIndicatorView{
    ...
    _selectIndicatorView = [[DeviceView alloc] initWithFrame:CGRectMake(0, (44.0f + [UIDevice table]), [[UIScreen mainScreen] bounds].size.width, 64)];
...
- (SetTitlePanoramicView *)setGroupnameView{
    ...
    if(!_setGroupnameView){
        ...
        _setGroupnameView = [[SetTitlePanoramicView alloc]initWithFrame:CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height)];
    }
...
- (UIView *)defView{
    ...
    if(!_defView){
        ...
        _defView = [[UIView alloc]initWithFrame:CGRectMake(0, (44.0f + [UIDevice table]), [[UIScreen mainScreen] bounds].size.width, 400)];
        //: _defView.hidden = YES;
        _defView.hidden = YES;
        ...
        defImg.image = [UIImage imageNamed:StringFromNightlongData(kStr_mannerTitle)];
        ...
        emptyTipLabel.textColor = [UIColor isView:StringFromNightlongData(kStr_voterVesselRegularData)];
        ...
        emptyTipLabel.text = StringFromNightlongData(kStr_titleBrokenValue);
...
Byte * NightlongDataToCache(Byte *data) {
    int learnAnimal = data[0];
    int friendlyPit = data[1];
    Byte normFeat = data[2];
    int northernEnemy = data[3];
    if (!learnAnimal) return data + northernEnemy;
    for (int i = northernEnemy; i < northernEnemy + friendlyPit; i++) {
        int value = data[i] - normFeat;
        if (value < 0) {
            value += 256;
        }
        data[i] = value;
    ...
    data[0] = 0;
    data[northernEnemy + friendlyPit] = 0;
    return data + northernEnemy;
}

NSString *StringFromNightlongData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)NightlongDataToCache(data)];
}
